...
public sealed class ProviderHealthRegistry
{
    ...
    // Кулдаун негодного auth стороннего провайдера (плохой/протухший API-ключ, P29). Длиннее
    // квоты: ключ не чинится сам — только сменой в конфиге, поэтому короткий TTL гнал бы каждый
    // ход на мёртвый ключ до конца смены. Не бесконечный: fail-open «все шаги цепочки в кулдауне →
    // берём остывшего» сохраняется, а Clear по успешному ходу снимает, если ключ починили.
    public const int AuthCooldownMinutes = 360;
    ...
    // Сторонний провайдер с негодным auth (401/протухший ключ, P29) — тот же кулдаун, но с самым
    // длинным TTL. Как и квота, только для СТОРОННИХ провайдеров: здоровье подписок пула Claude
    // ведёт ClaudeSubscriptionPool.MarkAuthDead (постоянная пометка, без TTL).
    public void MarkAuthDead(string? key)
        => MarkUntil(key, AuthCooldownMinutes, FallbackErrorClass.AuthFailure);
