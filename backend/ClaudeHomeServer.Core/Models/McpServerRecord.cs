...
/// Отдельного значения McpServerSource НЕ заводим: откат кода на старую версию должен читать
/// mcp-servers.json, а неизвестного enum-члена она не знает. Раньше такая запись уносила в
/// .corrupt весь файл; после поэлементного разбора (JsonFileStore, коммит 132f2007) она стоит
/// только себя — но это по-прежнему молча исчезнувший у человека сервер, а не читаемая запись.
/// </summary>
...
public class McpServerRecord
{
    ...
    /// <summary>
    /// Доезжать ли в чаты ВНЕ проектов: ось «чаты вне проектов» в allow-модели. Дефолт false —
    /// сервер не появляется во внепроектных чатах, пока владелец явно не разрешит.
    /// </summary>
