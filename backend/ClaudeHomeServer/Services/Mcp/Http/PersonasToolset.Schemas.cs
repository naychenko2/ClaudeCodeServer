...
public sealed partial class PersonasToolset
{
    ...
    internal static IReadOnlyList<McpToolSchema> ManageHeadTools(bool inProject)
    {
        ...
        // Свои модели по уровням (ADR-007 §2): проброс на http-ветке, в stdio-ветке есть.
        // Пустая строка — сбросить к наследованию от специальности; параметр не передавать
        // — не менять. Своя валидация значений не нужна: схема не должна отвергать легитимное
        // (урок enum специальностей), проверяет бэкенд (IsValidTierCell).
        updateProps["tierStrong"] = Str("Своя модель для сильных задач: id модели или preset:{id}; "
            + "пустая строка — сбросить к наследованию от специальности; параметр не передавать — не менять");
        updateProps["tierMedium"] = Str("Своя модель для средних задач: id модели или preset:{id}; "
            + "пустая строка — сбросить к наследованию от специальности; параметр не передавать — не менять");
        updateProps["tierWeak"] = Str("Своя модель для слабых задач: id модели или preset:{id}; "
            + "пустая строка — сбросить к наследованию от специальности; параметр не передавать — не менять");
