...
Исходники: [ADR-016](../adr/ADR-016-local-projects.md) (редакция коммита `c4dec062`) и
[спайк](local-projects-spike-2026-09.md) (§3 «Грабли», §5 «Что меняется»). Задача трекера —
`2b46cc52`. Все пути и строки ниже сверены с деревом ветки `feat/local-projects` на
...
ретранслятор и только онлайн; у агента нет долговременного состояния.

## 1. Что показала разведка по коду (опоры плана)

| Опора | Где сейчас | Что из этого следует |
|---|---|---|
| Выбор токена подписки | `ClaudeSubscriptionPool.Pick` (`Llm/ClaudeSubscriptionPool.cs:181`), зовут 8 мест | Шлюз берёт токен тем же `Pick`, своей ротации не заводит |
...
3. **Токен хода** — `TurnTokenService` в `Llm/Gateway`: в памяти (рестарт бэкенда = отзыв
...
4. **Шов канала исполнения** — `IDeviceExecChannel` в Core: реализует `Desktop` (ему
   принадлежат устройства и их авторизация), потребляет `Execution` (`RemoteProcessRunner`).
   Прямой ссылки `Execution → Desktop` нет.
5. **Потоковый канал исполнения** — отдельный WebSocket устройства
...
6. **Сайдкар ходит к шлюзу по HTTPS сервера** на адрес `/gw/t/{turnId}/…` с двумя
   авторизациями: токен устройства (кто звонит) и `X-Turn-Token` (какой ход). Шлюз сверяет,
   что ход привязан к этому устройству.
7. **Агент — `backend/ClaudeHomeServer.DeviceAgent`** (+ `.Tests`): кроссплатформенный
...
8. **Фич-флаг `local-projects`** (dark launch) закрывает создание локальных проектов и весь
   UI; шлюз для серверных проектов флагом не закрывается — у него свой тумблер
   `LlmGateway:Enabled` (false по умолчанию).
9. **Браузер → агент на `localhost`** (этап 4): билет доступа выдаёт сервер (короткий,
...
**1.2 (Д) Шлюз LLM и единая точка учёта пула.** Зависит от 1.1.
...
полный прогон до коммита. Сторожа G2, G9.
...
SSRF через выбор upstream. Готово: находки по severity, critical/high закрыты исполнителем
...
выключен → ход на подписке отказан понятным текстом. Готово: отчёт с шагами и
...
композицию); доступ по билету §2 п. 9; пути только под корнями, разрешёнными на машине,
...
- **Строго последовательно:** этап 1 первым (1.1 → 1.2/1.3 → 1.4); агент (2.2) — после
...
- **Параллельно:** 1.2 ‖ 1.3; 2.1 можно начать сразу после 1.1, не дожидаясь 1.2; 2.2 ‖ 2.3;
  3.1 ‖ 2.2/2.3 (после 2.1); 3.3 ‖ 3.4 ‖ 3.2; 4.3 ‖ 4.4 ‖ 4.5; 5.2 ‖ 5.3.
- Денис — узкое место: на нём 14 задач. Параллельные ветки для него — это отдельные
