...
// Стаб адаптера: запоминает ходы, ничего не стримит и не запускает процесс.
// Ответа модели не будет — статус сессии остаётся Working, но hosted-сервисы в Testing
// выключены, и контроллер-тесты статус не проверяют (им важен факт вызова, не результат).
