...
**Из каждой точки после стопа есть путь назад.** Провал ДО подмены бинарников (не снялся
снимок релиза — залоченный файл, нет места, антивирус на `.exe`) прод лежать не оставляет:
в `PublishDir` ещё прежняя сборка, агент поднимает её и проверяет health (без `X-Build` —
маркер там чужой), и только потом пишет `failed`. В сообщении итога прямо сказано, поднялся
прод или нужен человек.
...
> Отказ guard'а **после подхвата заявки** закрывает её в журнале (`phase: "failed"` +
> `result`): иначе заявка навсегда осталась бы `queued`, сервер отвечал бы 409 на все
> следующие и молчал инициатору. Единственный отказ до подхвата — занятый мьютекс: там
> заявкой занимается уже работающий агент.

**Коды возврата:** `0` успех · `1` провал · `2` отказ guard'а · `3` выкатка откачена.

**Останавливает агент только свой стек** — процессы `ClaudeHomeServer`,
`ClaudeHomeServer.Tray`, `ConPtyBridge`, **чей exe лежит в `PublishDir`**. Хостовой дев-стенд
(`dotnet run`), инспекционная копия бэкапа (`--inspect`) и инстанс полигона на :8080 живут
своей жизнью и выкаткой не задеваются.
...
| `-RequireBuildHeader` | — | требовать `X-Build` и при откате (в выкатке он требуется всегда) |
...
**В выкатке заголовок обязателен, поблажек нет.** Публикуемая сборка пишет маркер всегда
(агент — `build-id.txt`, бэкенд — `BuildIdProvider`), поэтому «`2xx`, но заголовка нет»
означает ровно одно: на порту отвечает **не наша сборка** — старый инстанс под внешним
Runner, чужой слушатель или кеширующий прокси, пока новая падает на старте по кругу. Такой
ответ гейт не засчитывает и уходит в автооткат.

Единственная поблажка — **откат на снимок**: старый релиз маркера может не нести (снимки,
снятые до появления `build-id.txt`). Там ответ без заголовка засчитывается; снять поблажку —
`-RequireBuildHeader`. Если в снимке маркера нет, агент **удаляет `build-id.txt` из
`PublishDir`** перед стартом: отсутствие заголовка честнее, чем маркер провалившейся выкатки
поверх откаченного кода.
...
- **Не зеркалит корень публикации при откате.** Снимок кладётся поверх (`robocopy /E`),
  зеркалятся только `wwwroot`, `mcp`, `mcp-dify`. Файл, появившийся в новой сборке и
  отсутствующий в снимке (новая dll, новый конфиг), после отката в `PublishDir` останется.
  Для .NET это обычно безвредно — лишнюю dll никто не грузит, — но «вернули ровно как было»
  это не значит. Единственное исключение сделано для `build-id.txt`: его агент убирает
  явно, чтобы `X-Build` не врал. Нужна побайтовая чистота — сноси `PublishDir` руками
  (кроме `data`, `logs`, `backups`, `certs`) и раскатывай снимок в пустую папку.
