...
public static class DefectRules
{
    ...
    // 1) Отказ при попытке создать дефект в статусе Done. Создание дефекта минуя
    // работу запрещено: иначе пропадает весь смысл бага как «работа, которую нашёл
    // наблюдатель и которую надо проверить». Покрывает инвариант Status == Done ⇒
    // есть Verification или Outcome == ClosedWithoutCheck со стороны создания.
    public static void EnsureNotClosedAtCreate(TaskItem task)
    {
