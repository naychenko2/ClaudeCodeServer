...
// Полнотекстовый fallback памяти (Этап 5, волна 2: вынесен в Core ради Dossiers).
// Раньше жил в `Services.Memory` — из-за этого Dossiers.DossierRecallService
// держала прямую ссылку на тип из чужой вертикали. Общий примитив Tokenize
// общий; дальше стеки расходятся ОСОЗНАННО (поведение recall менять нельзя),
// поэтому у каждого своя точка входа:
