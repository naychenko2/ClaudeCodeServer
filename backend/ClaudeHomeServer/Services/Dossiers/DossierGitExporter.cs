...
public sealed class DossierGitExporter
{
    ...
    internal const string ReadmeText =
        ...
        или задачи, приложение сохраняет цель изменения, принятые решения, отвергнутые варианты и
        грабли — и складывает их сюда само, в отдельную ветку рядом с кодом. Ветка остаётся только у
        вас, пока вы сами не отправите её в общий репозиторий: знания уезжают вместе с кодом, а не
        ...
        Каждая выгрузка перезаписывает содержимое ветки целиком, а идёт она сама — по мере появления
        новых записей. Ручные правки исчезнут при следующей выгрузке, незаметно. Ветка обычная во всём
        остальном: `git pull` и `git push` работают как всегда, рабочую папку выгрузка не трогает, в
        историю кода эти коммиты не попадают.
