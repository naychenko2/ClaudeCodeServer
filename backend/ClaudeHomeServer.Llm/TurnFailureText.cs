...
public static class TurnFailureText
{
    ...
    // Тумблер LlmGateway:Enabled выключен: маршруты шлюза отвечают 404, ход локального проекта
    // не стартует вовсе, а не падает посреди хода.
    public const string GatewayDisabled =
        "Шлюз локальных проектов выключен на сервере (LlmGateway:Enabled), ход на устройстве не запущен.\n\n"
        + "Попросите администратора включить шлюз.";
