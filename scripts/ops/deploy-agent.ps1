...
param(
    ...
    [int]$BackupTimeoutSec = 2400  # бэкап данных прода — секунды; 26.08 занял 834 с (93% прежнего
                                   # потолка 900) и объём растёт, поэтому 2400; жёсткие 2 минуты
                                   # из deploy80 роняли первую же боевую выкатку
)
...
function Invoke-DataBackup {
    ...
    # ~14 минут (834 с на 26.08) и объём растёт, потолок задаёт -BackupTimeoutSec. Делаем ДО остановки —
    ...
    if (-not $proc.WaitForExit($BackupTimeoutSec * 1000)) {
        try { $proc.Kill() } catch { }
        throw "бэкап не уложился в $BackupTimeoutSec с и был прерван (фактический ориентир: 834 с на 26.08, объём данных растёт; зависание дольше — повод смотреть, а не ждать)"
    }
