...
public class SmotrimProviderTests
{
    ...
    // Ключевой инвариант: 17 из 20 каналов каталога вещаются чужим плеером по
    // домену-реферреру. Признак должен приходить из ответа, иначе через месяц
    // список играбельных разъедется с реальностью.
    [Fact]
