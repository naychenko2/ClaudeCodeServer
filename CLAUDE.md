...
привязки, изолированные по владельцу из claim `sub`). stdio-файлы переехавших заморожены как
ветки отката; на stdio — `wsp`, `notifications`, `codegraph`, `desktop` (волна 3). Подключение per-ход
общее:
