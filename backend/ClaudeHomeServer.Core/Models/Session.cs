...
// Повод вызова раздачи волны (D1, ревью 2026-08-17). При снятых авто-волнах договор режима:
// следующая волна идёт только по явной кнопке человека, остальное — гейт-карточка. Нужен
// именно повод: кнопка «Запустить» и «докрут по состоянию после ответа на чужую карточку»
// приводят в один и тот же метод раздачи, а решения о гейте принимает TeamWaveService.
public enum TeamWaveTrigger
{
    // Человек явно запустил работу: «Запустить» на карточке плана или гейта, «Добавить
    // бюджет», «Продолжить», добавочная волна по его вводной. Гейт не нужен — решение
    // запускать уже принято нажатием.
    UserCommand,
    // Конвейер дошёл до раздачи по состоянию после решения человека: ответ на чужую
    // карточку (allow/keepPlan/keepFixing/answer), ответ текстом, выход из интервью
    // после закрытой волны. При AutoWaves == false вместо раздачи — гейт-карточка.
    StateCatchUp,
}
...
public class TeamImplementBudget
{
    ...
    // Потолки после расширения (волна 1 team-blocker-honest): Max + max(0, plan - left).
    // Заполняются на бэке в момент формирования снимка для фронта (BroadcastTeamImplementAsync),
    // когда известен план; 0 — плана нет (плашка бюджета скрывается). Используются плашкой
    // «план сверх остатка» — она показывает ЧЕСТНЫЕ новые потолки, а не размер плана (раньше
    // текст говорил «поднимется до N», где N — это план, и при ненулевом расходе числа
    // расходились — находка ревью c156193b).
    public int MaxWavesAfter { get; set; }
    public int MaxTasksAfter { get; set; }

    // Сколько нужно ДОБАВИТЬ к потолку, чтобы покрыть план сверх остатка. Единая точка
    // для плашки бюджета (TeamStateService.FillAfterBudgetAsync) и автоматического
    // расширения потолков при подтверждении плана человеком (TeamDecisionService):
    // две точки с одинаковой формулой Max + max(0, plan - left) ровно так родился
    // дефект c156193b — формула в TeamStateService считала от remainingWaves
    // (Math.Max(0, MaxWaves - WavesUsed)), а в TeamDecisionService — от plan напрямую,
    // и при ненулевом расходе числа разъезжались. Здесь формула одна, и она же
    // покрыта тестом TeamBudgetTests.PlanShortfall_НенулевойРасходУчитывается (M1
    // фикс-волны 4 team-blocker-honest).
    // Защита от отрицательного остатка — max(0, max - used); защита от плана меньше
    // остатка (нас уже устраивает текущий потолок) — внешний max(0, …). На легаси-сессии
    // счётчики могут быть неинициализированы, и без нижнего Max(0, …) ушла бы
    // отрицательная дельта и вместе с ней арифметика Max += delta.
    public static int PlanShortfall(int max, int used, int plan) =>
        Math.Max(0, plan - Math.Max(0, max - used));
