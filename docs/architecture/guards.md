...
| `ProjectMapHygieneGuardTests` | `CLAUDE.md` ≤ 520 строк, без мёртвых ссылок/@-импортов | `docs/research/claude-md-cleanup-2026-09.md` (уборка 1013→475) | качество/длина секций не меряются; «токены» — оценка |
...
- **Как ловит:** unit-теории над статикой `CoordinatorWriteGuard`: `IsShellTool`
  ...
  `LooksLikeFileWrite` — позитив (16 команд записи: heredoc, `echo >`, `printf >>`,
  `tee`, `sed -i`, PowerShell `Set-Content`/`Add-Content`/`Out-File`/`New-Item`/
  `Copy-Item`, `awk ... >`, `mv`, `cp`, голый редирект `'using System;' > Program.cs`)
  и негатив (`dotnet build/test`, `git status/diff`, `ls`, `npm run build`,
  редирект лога сборки, null/пусто).
...
- **Инвариант:** IL-скан, включённый в сторожей границ (волна 1, задача `8beee75e`),
  обязан находить все известные швы, которые прежние рефлексионные сторожи не видели
  (3 из них живут во вложенных типах: async-state-машинах и замыканиях) — иначе
  сторож выглядит работающим, а стал декоративным. Швов было семь, живых проб
  осталось шесть.
- **Источник:** `docs/research/il-boundary-scan-2026-09.md` (§«Вердикт по слепым пятнам»
  + таблица «шов → где нашлось»): «все семь швов, которые прежние волны объявляли
  „вслепую, для будущего IL-скана“, IL-скан действительно находит». Шов
  «Tasks → Hubs.TaskHubExtensions» снят после Этапа 5 Ф4 (extension-метод удалён),
  поэтому в массиве `checks` шесть строк, а не семь. Комментарий самого теста
  адресует источник к секции CLAUDE.md «Известное ограничение» — в карте проекта
  такой секции больше нет, ссылка мёртвая.
...
- **Источник:** `docs/research/claude-md-cleanup-2026-09.md` (инвентаризация и уборка
  2026-09-22: 1013 → 475 строк по таблице «Статус фазы 1»; комментарий самого
  теста называет 477 — числа замеров расходятся; правило «держи карту компактной»
  висело в шапке без обратной связи — «правило без гейта — пожелание») + `docs/features/
...
- **Как ловит:** теории с InlineData по каждому классу адресов, включая
  границы (172.31.255.254 vs 172.32.0.1, приватная проба 192.168.1.1 при
  публичной 192.169.0.1 сразу за блоком, benchmark 198.18/15: 198.18.0.1 и
  198.19.255.255 — приватные, соседние 198.17.255.255 и 198.20.0.1 — публичные,
  223.255.255.255 vs 224.0.0.1) и NAT64-представления
...
- **Как ловит:** два независимых механизма: (1) **строковый скан исходников**
  (IndexOf/Regex по телам методов, строки, начинающиеся с `//`, выкидываются, НЕ
