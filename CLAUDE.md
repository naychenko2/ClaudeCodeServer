...
(`Session.VoiceStyle`, флаг `voice-digest`): `talk` — всё вышеописанное; `digest` — ответ
...
БЕЗ озвучки у длинных ответов (`LongAnswerSectionText`, флаг доезжает через
`LlmSessionContext.LongAnswerDigest`): плашка «Коротко» в ленте + кнопка разового чтения. Озвучка — `POST /api/tts`
