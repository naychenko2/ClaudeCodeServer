...
вложенных типов в `BoundaryIlScanner` — краснеет и он, и Theory-сторожа:
общий сбор типов вынесен в `BoundaryIlScanner.CollectAllReferencedTypes`,
его зовут И `SubsystemBoundaryTests`/`RootSubsystemBoundaryTests`,
И `IlBoundaryRegressionTests`. Подмена в сканере роняет оба гейта одним
движением; дыра «сторож выглядит рабочим, а декоративен» закрыта
