...
// Глобальная страховка от «белого экрана»: любое необработанное исключение при
// рендере (ChunkLoadError при недоступности сервера статики, сбой SW, TypeError в
// компоненте) React иначе гасит демонтажем всего дерева — и пользователь видит
// пустую страницу без единого слова. Здесь ловим и показываем понятную заглушку.
//
// Признак «не удалось догрузить код» (лёг фронт-сервис / выкатка новой версии) —
// сообщение обычно ChunkLoadError или «Failed to fetch dynamically imported module».
...
export class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: unknown, info: ErrorInfo) {
    // В консоль — для диагностики (стек компонента + сама ошибка)
  ...
  render() {
    ...
    const title = this.state.chunk ? 'Не удалось загрузить приложение' : 'Что-то пошло не так';
    const hint = this.state.chunk
    ...
    return (
      <div style={{
        minHeight: '100vh', background: C.bgMain, fontFamily: FONT.sans,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 420, gap: 12 }}>
          ...
          <button
            onClick={this.reload}
            style={{
              marginTop: 8, background: C.accent, color: C.onAccent, border: 'none',
              borderRadius: R.lg, padding: '9px 18px', fontSize: 14, fontWeight: 600,
