...
| Категории делегирования (8 шт.) | `src/tools/delegate-task/*-categories.ts` | [categories.md](translations/categories.md) — короткая таблица; [category-profiles.md](translations/category-profiles.md) — полные профили | Таблица идёт в секцию «ДЕЛЕГИРОВАНИЕ» промпта персоны-исполнителя (`TaskExecutionService`, константа `OmoPrompts.DelegationCategories`); профили в промпт НЕ попадают — `PersonaAgentFileSync` кладёт их файлом `.claude/delegation-categories.md` в рабочую папку (константа `OmoPrompts.DelegationCategoryProfiles`), исполнитель читает по требованию. В постановке — **абсолютный** путь (`Read` относительный не принимает), файл гарантируется перед запуском (`EnsureCategoryProfiles`, мимо троттлинга `SyncOwner`). Свой файл узнаём по шапке-маркеру: одноимённый пользовательский не перезаписываем и ссылку на него не даём |
