...
public static class TeamImplementPrompts
{
    ...
    public static string ClarifyInterviewTurn(string reason) =>
        "[Режим «Командная реализация»] Волны на паузе — ты сообщил, что требования неясны: " +
        $"«{FirstLine(reason)}». " + InterviewOpeningLine + " " +
        "Задай вопросы инструментом AskUserQuestion (не текстом), а после ответов человека " +
        "дай маркер работы — команда пересоберёт план и покажет его на подтверждение.";
