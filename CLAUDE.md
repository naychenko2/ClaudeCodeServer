...
7 известных швов проверяются на видимость при каждом прогоне. Сломай обход
вложенных типов в `BoundaryIlScanner` — краснеет он один, 39 сторожей границ
остаются зелёными. **Чего он НЕ сторожит:** подмену самого ВЫЗОВА сканера в
сторожах (`SubsystemBoundaryTests`/`RootSubsystemBoundaryTests` собирают типы
через `AllMethodsWithNested`, и если там вернуться к `GetMethods`, регрессия
останется зелёной). Закрытие этой дыры — отдельная задача шага 5.
