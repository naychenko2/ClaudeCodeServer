...
public class Session
{
    ...
    // Чат в архиве: убран из списка, счётчиков непрочитанного и уведомлений, но НЕ удалён —
    // виден в архивном виде списка и открывается как обычно. Хранится момент архивации, а не
    // голый флаг: от него считается опциональная ретенция (Session:ArchiveRetentionDays), а
    // «когда убрал» — единственный вопрос, который к архивному чату задают. null — чат обычный.
    // Инварианты: архивация НЕ двигает UpdatedAt (это настройка, а не активность), а сообщение
    // человека в архивный чат возвращает его из архива (SendMessageAsync).
