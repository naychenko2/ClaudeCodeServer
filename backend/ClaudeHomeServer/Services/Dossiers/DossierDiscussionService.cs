...
// несоразмерно пользе — потому конспект, а не лента. Потребитель — выгрузка в ветку
// (DossierGitExporter.ExportAsync → EnsureAsync), но снятие происходит ТОЛЬКО по
// явной команде человека — ручной выгрузке (POST /dossiers/export; отправка push
// ветку не пересобирает — в ней едет то, что сняла последняя выгрузка): снятие —
// вызов модели на каждый чат, и фоновая автовыгрузка его не запускает (разбор 23.08) —
// она везёт уже снятые конспекты из стора, недостающие догоняет ближайшая ручная выгрузка.
