...
| Ходы массово встали | спаны `chat.turn` (трейсы), разрез по `chat_id` | ≥ 3 ходов дольше 15 мин в ОДНОМ чате за 90 мин | warning |
...
`deployment.environment` + `chat_id`, порог «три и более» применяется к каждому чату отдельно.
...
шаг (урок прежних редакций в силе). Окно и порог связаны арифметикой: три хода по 15 минут
требуют минимум 45 минут чистого времени, так что прежние полчаса сделали бы условие
недостижимым в принципе — отсюда 90 минут, двойной запас на паузы между ходами. Фильтр
`chat_id EXISTS` обязателен: спаны без тега схлопнулись бы в одну безымянную серию, и разные
чаты снова суммировались бы. Калибровка по семи суткам трейсов (23 хода дольше 15 минут на
бою): максимум в одном чате — три подряд, то есть за неделю правило зажглось бы ровно один
раз. Если и это окажется шумом, поднимать надо ПОРОГ (4 хода), а не границу: 15 минут уже
вдвое больше типичного агентного хода с tool-use.
