...
MCP-сервером `mcp/desktop-server` (шесть инструментов `desktop_*`). В первой волне сделана
серверная половина; клиента, UIA-снапшота, кадра и расширения ещё нет. Инварианты:
...
«сказал → пауза 2 с → отправка → ход → ответ вслух → снова слушаю», автомат чистым редьюсером
...
ходы идут прямым HTTP-вызовом Ollama мимо claude CLI, ответ за секунды, фронт не меняется.
