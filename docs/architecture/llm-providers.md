...
TCP-коннект к адресу из `HTTPS_PROXY`/`HTTP_PROXY`/`ALL_PROXY`. Спрашивается **только** при
...
- **один повтор** той же пары через `EgressRetryDelay` (5 с) — цепочка не двигается,
  `substitutions` не тратится, кулдаун провайдеру не ставится;
- повтор не помог — `FailEgressAsync`: текст `TurnFailureText.EgressDown` («Нет связи с
  интернетом… Смена модели тут не поможет») плюс финальный `result` со `subtype: error`.
