...
// CHAT_GUTTER_L — жёлоб СЛЕВА от колонки сообщений: место под индикатор ожидания
// (аватар 28px + расходящиеся кольца «Эхо»), вынесенный за левый край ленты. Геометрию
// области прокрутки по нему считает chatGutter.ts (marginRight компенсирует жёлоб,
// поэтому колонка сообщений и композер остаются по центру окна при любом размере),
// здесь величина живёт как размерный токен — от неё же считается CHAT_COLUMN_W ниже.
// Аватар сдвинут в жёлоб на -(28+10)=38 (см. ChatPanel), центр на 14 от левого края
// жёлоба; кольца расходятся до radius 26 — нужен запас, чтобы их не резал клип
// области прокрутки (overflow-x: hidden). 28+10+14 ≈ 52 — бери отсюда.
export const CHAT_GUTTER_L = 52
