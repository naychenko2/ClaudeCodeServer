...
#   10 ai-home-agent install вернул ошибку (каталог версии уже создан, дальше — сам агент);
#      его коды 20 и 21 («запустится при следующем входе», «сервер пока не принял») — это
#      успех с предупреждением, скрипт выходит с 0
...
log "==> $AGENT_BIN $*"
# Код берём явно: после `if ! cmd` в $? лежит уже отрицание, а не код install
rc=0
"$AGENT_BIN" "$@" || rc=$?
# 20 и 21 — агент установлен, но есть что сказать человеку (коды InstallExitCodes агента)
case "$rc" in
    0) ;;
    20) log "==> агент установлен, но сейчас не запущен; он запустится сам при следующем входе в систему" ;;
    21) log "==> агент запущен, но сервер его пока не принял; агент продолжит попытки сам (журнал — $XDG_DATA_HOME/ai-home-agent/logs)" ;;
