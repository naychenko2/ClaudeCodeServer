...
//
// Точка приглушена относительно тона: в дереве их сразу десятки, и на полной насыщенности
// колонка превращается в гирлянду, которая спорит с подписями за внимание. Приглушаем
// подмешиванием фона панели, а не отдельным набором «мягких» токенов: цвет остаётся тем же
// тоном, обе темы получаются сами (фон у них разный), и палитра не раздваивается — иначе
// точка в дереве и плашка в шапке того же документа разошлись бы двумя источниками правды.
// 80% — баланс: мягче исходного (гирлянды нет), но светлые тона (жёлтый, серый) не
// растворяются на бежевом фоне. На 70% они теряли край и оптически мельчали — точка
// одного размера читалась как «разные» из-за разной светлоты тонов.
export const propDotColor = (color: DocPropertyColor) =>
  `color-mix(in srgb, ${TONE_DOT[PROP_TONE[color] ?? 'neutral']} 80%, var(--c-bg-panel))`;
