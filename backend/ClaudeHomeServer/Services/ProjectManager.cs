...
public class ProjectManager
{
    ...
    public const string BuiltInSystemPrompt =
        ...
        "Если пользователь просит сгенерировать, нарисовать, создать или изобразить что-либо визуальное (изображение, картинку, рисунок, арт, фото, иллюстрацию), видео, аудио или музыку — используй MCP-сервер fal-ai: подбери модель (recommend_model или search_models), запусти задачу (submit_job / run_model) и верни результат как есть. Ссылки на изображение/видео/аудио из ответа fal-ai отображаются в чате автоматически — НЕ открывай их в браузере и не используй для показа playwright, WebFetch или другие инструменты; после вызова fal-ai просто заверши ход. Никогда не рисуй ASCII-арт вместо настоящей генерации.\n\n" +
        ...
        // Широкие ASCII-схемы «разъезжаются» при переносе строк по ширине чата:
        // узкие рисуем ASCII, широкие/с развилками — виджетом
        "Схемы и диаграммы по умолчанию рисуй ASCII-артом в кодовом блоке: ориентация сверху-вниз, ширина до ~60 символов, чистый ASCII (+ - | и пробелы), без Unicode box-drawing (─ │ ┌ ┐). Если схема широкая или с развилками и не вписывается в ~60 символов — рисуй её виджетом (инструмент widget_show, SVG/HTML): там линии — это элементы с фиксированными координатами, они не зависят от шрифта и не «разъезжаются» при переносе строк.\n\n";
