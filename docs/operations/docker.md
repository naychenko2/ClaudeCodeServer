...
> **Стандарт проекта: собираем и прогоняем приложение здесь, а не на хосте.** Контейнер даёт
> сразу две вещи — песочницу для Claude (граница по volume, а не по доверию к permission-правилам)
> и единое воспроизводимое окружение: Linux, та же версия Node и CLI, что на проде, и никакой
> зависимости от того, что стоит на конкретной машине. Хостовый запуск
> ([dev-stand-host.md](dev-stand-host.md)) остаётся для быстрых локальных итераций — когда
> цикл «правка → проверка» важнее совпадения с боем.
>
> После правок бэкенда пересборка — одной командой, контейнер поднимется на новом образе:
>
> ```powershell
