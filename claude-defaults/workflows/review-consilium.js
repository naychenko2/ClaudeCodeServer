...
// пустое значение (например `findings: []`), чтобы модель не опускала поле
// вместо валидного «чисто». Держим рядом со схемами — правишь `required`,
...
Если по твоей оси всё чисто — верни пустой массив находок и скажи об этом в summary. Не выдумывай проблемы ради галочки. Отвечай по-русски.${requiredFieldsReminder(FINDINGS_SCHEMA)}`
...
В сомнении не отбрасывай (verdict=сомнительно). Отвечай по-русски.${requiredFieldsReminder(VERDICT_SCHEMA)}`,
...
Дай итоговый вердикт: что блокирует мерж (критичные/серьёзные), что можно на потом, и рекомендацию. Будь конкретным и практичным, без воды. Отвечай по-русски.${requiredFieldsReminder(SYNTH_SCHEMA)}`,
