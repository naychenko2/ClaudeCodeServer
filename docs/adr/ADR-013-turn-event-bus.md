...
Что болит: на каждое новое наблюдение за ходом (`tool/result`, `turn/started`) приходится
заводить поле контекста и парный sink; per-owner изоляции на этих точках нет, она
обеспечивается тем, что sink смотрит на `IHubContext` или на сессионный scope, а у хода
вне HTTP (фоновый ход, ход-исполнитель задачи) ни того, ни другого нет — и добавление
нового sink'а снова будет требовать частного решения.
...
- **`ITurnNotification`** — факт свершился (fire-and-forget). Подписчик стоит в стороне от
  ...
  падает. Это нормальная семантика наблюдения, и у `tool/result`, `subagent/completed`,
  `turn/completed` она единственно правильная.
...
Семь событий (в исходной постановке перечислено шесть; `subagent/completed` добавлен на
этапе 0 как седьмое — без него переезд `SubagentRunSink` на шину в этапе 1 невозможен,
см. ниже).

| Событие | Вид | Producer | Consumer этапа 1 | Зачем |
|---|---|---|---|---|
| `turn/started` | Notification | `SessionManager` (ход принят к исполнению, текст пользователя известен, попыток ещё не было) | (этап 2) | Факт старта хода. Первая точка, на которой внешнему наблюдателю имеет смысл заводить per-turn состояние |
| `turn/attempt-started` | Notification | `FallbackLlmSessionAdapter` (начало конкретной попытки хода конкретной парой «модель × провайдер») | (этап 2) | Попыток у одного хода несколько (цепочка фолбэка). Нужно там, где сейчас `LlmSessionAdapterFactory` ведёт счётчик трат per-attempt |
...
| `tool/result` | Notification | `ClaudeSession` (наблюдён результат инструмента, в том числе ошибочный) | (этап 2) | Факт `tool_use` → `tool_result`. Картина ещё не определена, в v1 публикуется только для отладки/телеметрии |
...
| `turn/completed` | Notification | `FallbackLlmSessionAdapter` (`finally` цикла попыток) | `TurnRunLog` (этап 1: переезд записи паспорта хода с `finally` на подписчика) | Паспорт хода. `Outcome` (success/failed/egress_down/interrupted/cancelled/crashed), `ErrorClass` (`TurnErrorClassifier`) у неуспешного исхода, `Passport` (полный `TurnRunPassport`) |
...
- **`tool/result`/`turn/started`/`turn/attempt-started` не имеют UI-представления в v1.**
  Если в этапе 2 такое представление появится — оно придёт как НОВЫЙ тип `ServerMessage`,
  а не как проброс шины наружу.

Если в этапе 2 понадобится UI-уведомление о событии шины — заводится точечный подписчик
`turn/started` → `RecallManifestMessage` (как уже спроектировано для `ManifestItems`
`prompt/assembling`), а не «шина → фронт».
...
Конкретные consumer'ы для `turn/started`, `turn/attempt-started`, `tool/result` — картина
ещё не определена, в v1 публикуются только для отладки/телеметрии. Вынос
`DecidePermissionAsync` на шину — отдельный вопрос (см. §«Не трогаем в v1»). Состав
подписчиков `prompt/assembling` и `prompt/assembled` — по результатам этапа 1
