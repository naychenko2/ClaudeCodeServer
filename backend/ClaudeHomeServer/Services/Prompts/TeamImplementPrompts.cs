...
public static class TeamImplementPrompts
{
    ...
    // Волна 6 (живая приёмка волны 5): ход оборван технически ДО того, как координатор успел
    // ответить — рестарт сервера, упавший процесс, таймаут провайдера. Раньше та же ситуация
    // тонула в SilentPlanningStallDetails/ClarifyStallDetails («не понял вводную»/«уточнения не
    // пришли») — формулировка отправляла человека переформулировать задачу, хотя причина не в
    // ней. Текст согласован с владельцем продукта.
    public static string TurnInterruptedDetails() =>
        "Ход прервался — сервер перезапускался (или процесс модели оборвался посреди ответа). " +
        "Вводная не потерялась и координатор ни при чём: нажмите «Повторить» у прерванного " +
        "сообщения выше — команда продолжит с того же места.";
