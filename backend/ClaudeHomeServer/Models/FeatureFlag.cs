...
public static class FeatureFlagKeys
{
    ...
    // Контекст чата: материалы, закреплённые за чатом явной кнопкой (полоса вкладок правой
    // половины сплита). Флаг гейтит и серверную часть — тул context_list в составе
...
public static class FeatureFlagCatalog
{
    ...
    [
        ...
        // Контекст чата (материалы правой половины сплита): вкладки материалов у чата плюс
        // серверная половина — инструмент context_list и подсказка хода про приложенные
        // материалы. Одним флагом, потому что вкладки без инструмента обещали бы агенту
        // то, чего он не видит.
        new FeatureFlagDefinition(
