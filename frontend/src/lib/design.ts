...
// CHAT_GUTTER_L — жёлоб СЛЕВА от колонки сообщений: место под значок ожидания
// («домик» с дымком), вынесенный за левый край ленты. Геометрию области прокрутки
// по нему считает chatGutter.ts, здесь величина живёт как размерный токен —
// от неё же считается CHAT_COLUMN_W ниже, а константа не может знать про хук.
// Значок 19px вынесен на 29px влево, дым уходит ещё на 7px — 40 с запасом.
export const CHAT_GUTTER_L = 40

// CHAT_COLUMN_W — сколько лента чата занимает ПО ГОРИЗОНТАЛИ целиком, вместе с
// обвязкой: жёлоб слева + колонка чтения + место под полосу прокрутки справа +
// внешний отступ, который гасит перекос «жёлоб против полосы». Ширина полосы
// сокращается (см. gutterBox), поэтому итог не зависит от системы: CHAT_MAX_W + 2×жёлоб.
//
