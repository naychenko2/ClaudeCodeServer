...
Не конфликтует с общим планом интеграции — просто добавляется опциональный `personaId` в
mapping. Этапы:

1. **MVP** (#2 + #4 из общей таблицы) — pure notifications без персон.
2. **V1** (+ #1) — permission push с кнопками.
3. **V1.5** — кейс B (уведомления от имени персон) — дёшево, переиспользует prompt builder.
4. **V2** — кейс A (личка с глобальной персоной) — самый сильный кейс, требует команд
   управления.
5. **V2.5** — кейс C (проект-специфичные чаты).
6. **Потом** — кейс D, если появится команда.
