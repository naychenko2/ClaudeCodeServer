...
public class TeamEscalation
{
    ...
    // Название задачи (волна 1 team-blocker-honest): кэш на момент публикации, чтобы
    // карточка эскалации могла показать его в диалоге снятия — раньше подставлялся
    // заголовок самой карточки («Исполнитель застрял: …»), и человек не видел, какую
    // задачу закрывает (дефект 1430b732). Снимок, а не live-резолв: между публикацией
    // и решением задачу могли переименовать — карточка остаётся про исходный момент.
    // Setter: подтягивается в PublishTeamEscalationAsync одним резолвом для всех мест
    // создания, после самого создания (init был бы слишком узким).
    ...
    // Пояснение к СНЯТИЮ карточки не человеком (волна 1 team-blocker-honest): координатор
    // снял блокер сам — переписал задачу, ответил исполнителю или закрыл снятой — и карточка
    // погасла по факту. Текст идёт в подпись «Снят штабом: …» в ленте (Кира подхватит форму
    // в волне 3). Для кнопок человека остаётся null: карточка уже горела достаточно долго.
    public string? ResolutionNote { get; set; }
    // Сколько повторных напоминаний по этой карточке уже ушло (сторож ожидания, тик раз в
    // минуту): максимум два — через час после публикации и через четыре после первого.
    // Живёт на карточке в истории чата, чтобы переживать рестарт сервера и после него не
    // начинать оклик заново.
    public int RemindersSent { get; set; }
    // Момент последнего напоминания — от него считается интервал до следующего.
...
public static class TeamEscalationActions
{
    ...
    // Карточка WaveStalled БЕЗ «Снять» (D2, ревью 2026-08-17; D3, приёмка круга 2): таковы
    // карточка мёртвой зоны конвейера (TeamWaveService.CheckDeadZoneStall — TaskId нет, ветка
    // drop ничего не снимала бы) и карточка таймаута волны с несколькими молчащими под-задачами
    // (одна кнопка не может снять их все — снимать пришлось бы первую попавшую). Подпись, не
    // отражающая последствие, хуже отсутствия кнопки; «Перезапустить» и «Завершить» работают
    // при любом составе.
    public static IReadOnlyList<TeamEscalationAction> WithoutDrop() =>
    [
        new("restart", "Перезапустить"),
        new("finish", "Завершить"),
    ];
