...
// Нижняя граница ШИРОКОГО планшета: [TABLET_WIDE_MIN, TABLET_MAX] — формально ещё
// планшет, но места там столько же, сколько на узком десктопе, и компактные ветки
// раскладки только вредят.
//
// Полоса 601–1199 задумывалась под планшет в районе 700–800, а под неё попадает и
// MatePad 11.5 S с его 1120 CSS (замер, см. docs/design/target-devices.md) — до
// TABLET_MAX ему не хватает 79px, и он получал мобильную механику: шапка чата шире
// колонки сообщений на 37px с каждой стороны, гит-бар свёрнут по умолчанию, панель
// поверх контента. Это не три бага, а один порог, применённый к экрану, для которого
// он не задумывался.
//
// 1000, потому что боковая панель в потоке — COL_DEFAULT = 340: контенту остаётся
// 660, и это ещё полноценная двухпанельная работа. Ниже — панель начинает
// доминировать, и компактные ветки снова правы.
export const TABLET_WIDE_MIN = 1000;
...
// шириной `w` это даёт порог окна `w / PANEL_INLINE_MAX_SHARE`.
//
// Треть, а не 0.3: доля 0.3 считалась от панели 300px (порог ~1000px), но панель
// давно шире — COL_DEFAULT = 340, и порог незаметно уехал на 1133px. MatePad 11.5 S
// с его 1120 не дотягивал ТРИНАДЦАТИ пикселей и получал drawer поверх чата при
// полностью пустой правой половине экрана. Треть возвращает панели 340 порог 1020
// и заодно перестаёт зависеть от того, помнит ли кто-то про связь двух чисел.
// Широкие панели доля по-прежнему держит: COL_MAX = 560 уйдёт в поток только с 1680.
export const PANEL_INLINE_MAX_SHARE = 1 / 3;
