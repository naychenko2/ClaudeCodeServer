...
public class ReaderCharThresholdTests
{
    ...
    /// <summary>
    /// Граница порога: у <see cref="ShortPageHtml"/> извлекается ровно 173 символа текста
    /// (замер 2026-09-19 по самой фикстуре), поэтому порог 173 обязан пропускать страницу,
    /// а 174 — отвергать. Обе стороны в одном тесте: если разбор фикстуры съедет, упадут оба
    /// случая сразу и причина будет видна, а не спрячется за односторонним ассертом.
    /// Прежние 175/176 брались не отсюда: 175 — это замер ЖИВОЙ example.com из коммита
    /// 325d96a0, а фикстура ей не равна (заголовок h1 в извлечённый текст не попадает).
    /// От переносов строк длина не зависит: AngleSharp сводит CRLF к LF ещё при разборе.
    /// </summary>
    [Theory]
    [InlineData(173, true)]
    [InlineData(174, false)]
