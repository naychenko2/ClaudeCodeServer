...
public static class TurnFailureText
{
    ...
    // к нему нет. Раньше тут был тихий срез в 200K — для длинного чата это мина (контекст
    // сотни тысяч токенов в 200K не влезет, человек получит переполнение вместо причины).
    // Формулировка уводит от ложного «сменить модель»: окно 1M — свойство подписки, не модели.
    public const string Window1MUnavailable =
        "Окно 1M сейчас недоступно: подписок с доступом к нему нет. Разговор не поместится в стандартные 200 тысяч токенов.\n\n"
        + "Дождитесь сброса лимита или продолжите чат на другом провайдере. "
        + "Смена модели внутри Claude не поможет: окно 1M есть не на всех подписках.";
