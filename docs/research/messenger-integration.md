...
> ## ⛔ Max закрыт для этой инсталляции (проверено 29.07.2026)
>
> **Регистрировать ботов в Max могут только верифицированные юрлица РФ.** С августа 2025
> платформа перевела ботов и mini-apps в бизнес-режим: ИП, самозанятые и физлица
> не допускаются ([разбор изменений](https://habr.com/ru/articles/951326/),
> [офиц. требования](https://dev.max.ru/docs/chatbots/bots-coding/prepare)). Модерация
> обязательна даже для бота, которым пользуется только автор. Юрлица нет — путь непроходим,
> и упирается он в самый первый шаг: без токена не написать даже заглушку.
>
> Попутная правка факта: **MasterBot существует** (аналог BotFather, создание бота идёт
> через него) — прежняя редакция этой доки утверждала обратное. Но доступ к нему — за той же
> верификацией.
>
