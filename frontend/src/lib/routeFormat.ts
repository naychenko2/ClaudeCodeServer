...
export function rateUnknownModel(model: string): string {
  return `Модель «${model}» не найдена среди доступных. Выберите её в списке раздела «Поставщики моделей».`;
}
...
export const RATE_PRESET_EMPTY =
  'Не указан пресет — выберите его в списке, и ссылка подставится сама.';
export function ratePresetMissing(id: string): string {
  return `Пресета «${id}» нет среди доступных — выберите пресет в списке.`;
}
