...
// Edit — правка плана человеком (кнопка «Изменить план»): сервер сам пересобирает план
// с текстом правки — карточка гаснет как заменённая, публикуется версия vN+1. Ход
// координатору на этот путь не выдаётся: перепланирование не зависит от поведения модели.
public enum TeamPlanDecision { Run, Reassign, Cancel, Edit }
