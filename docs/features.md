...
- **Область настраивается** (кнопка в шапке панели, диалог
  ...
  - **типы файлов** (дефолт `.md`) — из того, что панель умеет показывать:
    `.md .markdown .mdx .txt .rst` (`DocsIndexService.SupportedExtensions`). `.pdf`/`.docx`
    в этот список не входят: панель их не отрендерит, и предлагать их было бы обманом.

  Кандидатов считает бэкенд: папки с документами не глубже трёх уровней (без скрытых и
  `node_modules`) и файлы корня по **всем** поддерживаемым расширениям — иначе, сузив типы
  до `.md`, пользователь терял бы из виду свой же выбранный `NOTES.txt`. Есть ручной ввод
  папки: пустой пока папки в кандидатах нет, а завести область заранее — нормальное желание.
  Корень проекта папкой выбрать нельзя — это был бы обход всего репозитория.
