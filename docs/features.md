...
- **Область настраивается** (кнопка «Папки документации» в шапке панели, диалог
  [DocsScopeDialog.tsx](../frontend/src/pages/workspace/DocsScopeDialog.tsx)): соглашение о
  папке в проектах разное — `docs`, `wiki`, `documentation`, спеки рядом с кодом. Хранится в
  `Project.DocsFolders`: `null` — дефолт `docs/`, пустой список — только README. Кандидатов
  предлагает бэкенд (папки с `.md` не глубже трёх уровней, без скрытых и `node_modules`),
  плюс есть ручной ввод — папка без документов в кандидаты не попадает, а завести её заранее
  нормально. **`README.md` в корне входит в область всегда**: он не папка, и отключать его
  нечем. Корень проекта папкой выбрать нельзя — это был бы обход всего репозитория.
