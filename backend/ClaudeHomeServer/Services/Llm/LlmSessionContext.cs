...
public sealed record LlmSessionContext(
    ...
    // Просить у длинных ответов краткую выдержку (маркер <voice> последним блоком) — флаг
    // владельца voice-digest. Решение принимается по ВЛАДЕЛЬЦУ, не по свойствам хода, как
    // и BrowserEnabled выше; на состав инструментов не влияет вовсе — это только секция
    // системного промпта.
    bool LongAnswerDigest = false,
