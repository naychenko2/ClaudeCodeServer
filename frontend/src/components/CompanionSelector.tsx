...
interface Props {
  ...
  compact?: boolean;
  // Лестница деградации полосы: собеседник теряет подпись вторым — после усилия
  // (форма B2: усилие уже иконкой, собеседник схлопывается следом). Не путать с
  // compact=true (схлопнуть всё); для формы B2 compact=false, compactCompanion=true
...
export function CompanionSelector({ personas, agents, selectedPersona, selectedAgentName, onSelect, isMobile, dropUp = true, onCreateGroup, wide, compact, compactCompanion, maxLabelWidth }: Props) {
  ...
  // форма B2 лестницы полосы: усилие уже иконкой, собеседник сворачивается вторым;
  // у компонента compact=false, но триггер всё равно сворачивается
