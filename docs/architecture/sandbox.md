...
- **Изоляция local-процессов по памяти** (секция `Execution:Isolation`, по умолчанию выключена) —
  ...
  потокобезопасно) и повторяет запуск с теми же флагами, что `WrapperArgs`:
  ...
  — процесс запускается напрямую, один warning с причиной. При включённой изоляции в окружение ВСЕХ
