...
`Boundaries`) шесть (Video, Yandex, Reader, CodeGraph, Skills, Git) уже вынесены в
отдельные `.csproj` в рамках Этапа 3: сначала плановый мерж пилота (первые три),
затем по критерию готовности швов — CodeGraph (две волны), Skills (две волны) и
Git (уборка связанности + вынос). **Git — единственная из шести, которой вынос
потребовал предварительной уборки:** сначала `SafeJoin` → Core-примитив
`SafePath.Join`, `Models.User` вон из контракта через read-метод
`IForgejoAccountStore`, `Models/GitStatus.cs` → Core и возврат регистрации спинных
реакторов в `Program.cs`, и только потом `.csproj`.
Остальные **14** без нового критерия не трогаем (профиль правок против — 89%
коммитов трогают только спину, выигрыш Δ −50% не окупается), НО **новая
подсистема рождается отдельным `.csproj`**, если её потребности закрываются
существующими швами `Core` плюс максимум 1–2 новыми узкими интерфейсами. Иначе —
честный сигнал, что фича про спину, и она остаётся в Main. Сторожа границ
...
`Boundaries` сторожа не ловят ни один:** `SubsystemBoundaryCoverageTests` считает
покрытие через `Distinct()` по `NamespaceRoot`, и вторая запись о той же вертикали
молча схлопывается (поймано ревью выноса Git по расхождению числа тестов —
дублирующая запись добавляет лишний кейс в параметризованный тест границ). В `IlBoundaryRegressionTests` статического
