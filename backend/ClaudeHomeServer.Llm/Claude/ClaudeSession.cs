...
public class ClaudeSession : ILlmSessionAdapter
{
    ...
    // MCP-сервер сторожей чатов (watch_*): null — чат без владельца
        ...
        string? datasetId, PersonaAgentsContext? personaAgents = null)
    {
        ...
        try
        {
            ...
            if (hasWatch)
            {
                // Сервер сторожей чатов: единственная ветка — http (stdio-отката нет,
                // node-сервера не существовало). Сессия-вызыватель едет хвостом URL
                // (/mcp/watch/{sessionId}) — по ней тулсет резолвит владельца, проект и
                // будимый чат. alwaysLoad — по той же причине, что у tasks/notes: ленивое
                // подключение роняет первый вызов. Токен — фабрикой на каждую сборку
                // конфига (как у widgets, урок фазы 1).
        ...
        bool suppressTasksExecute, CancellationToken ct)
    {
        ...
        // Mermaid-схем, Dify, теги) пропадут. Same-process ход промпт живого процесса НЕ обновляет —
        // recall/подсказки на нём остаются со старта прогона (мягкая деградация).
        {
            ...
            if (_tasksMcp is not null && McpDelivered("tasks"))
            {
                ...
                // На неделегированном ходу делегирование — штатный инструмент, а не действие,
                // требующее отдельного согласия. На делегированном ходу разрешение противоречило
                // бы тексту executeHint про отказ — там цепочка намеренно обрывается
                var delegateHint = _currentTurnAgentDepth < 1
                    ? " Делегирование — штатный рабочий инструмент, а не действие, требующее отдельного согласия. " +
                      "Если работа уже обсуждена и понятно, кому её поручить, — создавай задачу и запускай исполнителя сам, " +
                ...
                var tasksHint =
                    ...
                    "Задача пользователя — только mcp__tasks__*." + columnsHint + executeHint + delegateHint + personaExecHint + resultHint + crossProjectHint;
            ...
            if (Info.ProjectId is not null)
            {
                var imagesHint =
                    ...
                    "репозитории (скриншоты, промежуточные кадры), клади в подпапку " + TreeExcludes.AttachmentsDir +
...
// ленивым new List<…>() прямо здесь).
        ...
        IReadOnlyList<PromptSectionDto>? truncated = null)
    {
        ...
        var id = SnapshotIdGenerator.NewPublicId();
        ...
        foreach (var block in content.EnumerateArray())
        {
            ...
            if (_transcriptOnServer && !isError && resultContent.Contains("Transcript dir:"))
            {
                ...
                if (m.Success)
                {
                    ...
                    Console.WriteLine($"[WorkflowWatcher] старт: dir={transcriptDir} allowed={TranscriptRoots.IsPathAllowed(transcriptDir)}");
