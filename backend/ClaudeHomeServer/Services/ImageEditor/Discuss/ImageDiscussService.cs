...
// фронт читает существующими событиями чата по возвращённому sessionId. Ручка — в модуле
// редактора, сюда она приходит через временный шов IImageDiscussStarter (ADR-018 §10.1).
//
...
public sealed class ImageDiscussService(
    ...
    FileService files,
    ILogger<ImageDiscussService> log) : IImageDiscussStarter
{
    public const int MaxTextChars = 20_000;

    public async Task<ImageDiscussOutcome> StartAsync(
        string ownerId, Project project, ImageDiscussInput input, CancellationToken ct)
    {
        ...
        var ext = input.AnnotatedExtension;
        ...
        if (session is null)
        {
            ...
            if (personaId is null)
                return ImageDiscussOutcome.Fail(ImageDiscussOutcome.Unavailable,
        ...
        files.WriteFileBytes(project.RootPath, attachment, input.Annotated);
        ...
        return ImageDiscussOutcome.Ok(new ImageDiscussResultDto(session.Id, created, attached));
    }
    ...
    private static ImageDiscussOutcome Invalid(string error) =>
        ImageDiscussOutcome.Fail(ImageDiscussOutcome.InvalidRequest, error);
}
