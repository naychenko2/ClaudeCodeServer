...
- **@упоминания (флаг `persona-mentions`)**: MCP [mcp/personas-server/index.js](mcp/personas-server/index.js)
  (`persona_list`, `persona_ask`) подключается ходам за флагами personas+persona-mentions,
  когда в контексте есть другие персоны (`BuildPersonasContext`: env `PERSONAS_*` +
  подсказка со списком «@handle — Роль (Имя)» в промпт). `POST /api/personas/ask` — one-shot
