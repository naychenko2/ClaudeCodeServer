...
public sealed record ProcessSpec
{
    ...
    // Чат хода: удалённый раннер привязывает к нему токен шлюза (ADR-016 §2). Остальные
    // раннеры поле не читают
    ...
    // Тяжёлый запуск: сборка или тесты — держит гигабайты и конкурирует за память с ходами
    // агентов. По этой метке BuildConcurrencyGate решает, занимать ли слот общего потолка.
    // Метка ставится ЯВНО на месте объявления spec, потому что по имени команды тяжесть не
    // определить: у `dotnet` build и `dotnet --version` одно имя, а git — всегда лёгкий.
    // Сама по себе метка НИЧЕГО не ограничивает — слот берёт вызывающий перед запуском
    // (Start синхронен, ждать слот внутри него значило бы заблокировать вызывающий поток).
