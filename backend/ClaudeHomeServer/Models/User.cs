...
public class User
{
    ...
    // Порядок списка = порядок в попапе. null/пусто — фразы не заведены (попап предложит
    // завести). Один набор на все чаты владельца — привязки к проекту сознательно нет.
    public List<string>? QuickPhrases { get; set; }
}
