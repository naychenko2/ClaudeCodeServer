...
гарантией на чужом дереве. Переведены оба потребителя: ватчер хода (439 слежек вместо 16 390)
и `FileWatcherService` (455 слежек и один экземпляр inotify вместо 37 682 в одиннадцати).
По символическим ссылкам примитив не ходит: петля `loop -> ..` раскручивалась бы до
ENAMETOOLONG, а потолок слежек её не ловит (тот же inode даёт тот же `wd`). Сбои
различаются намеренно: потеря событий лечится пересинхронизацией, смерть наблюдения —
пересозданием; отказ слежки на отдельном каталоге — частичный и пересоздания не поднимает
(ровно оно дало рекурсию 19.09). sysctl остаётся запасом, а не лекарством.
