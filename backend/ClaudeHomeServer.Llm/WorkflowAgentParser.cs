...
// Парсер agent-*.jsonl файлов workflow-транскриптов.
// Используется и WorkflowWatcher (реалтайм), и WorkflowController (REST).
//
// До волны 4C шага 4 этот класс нёс ДВЕ несвязанные сущности: парсер (ниже)
// и реестр корней транскриптов (`DefaultRoot`/`_extraRoots`/`ProfilesRoot`/
// `AddAllowedRoot`/`IsPathAllowed`/`AllowedRoots`). Реестр уехал в спину
// `Services.TranscriptRoots` — он наполняется тремя источниками из разных
// слоёв (константа `~/.claude/projects`; `LlmProviderRegistry.ProfilesDir`
// через `Program.cs`; `SandboxManager.ProfilesHostDir` через `DockerProcessRunner`),
// и класть его внутрь одной из питающих вертикалей — именно та ошибка, что
// породила цикл `Llm ⇄ Execution`. Парсер остаётся здесь: он потребляется
// только Llm (WorkflowWatcher/SubagentStreamWatcher/TranscriptProbe/ClaudeSession)
// и контроллером `WorkflowController` (через using).
