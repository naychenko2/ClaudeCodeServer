    ...
    HiggsfieldIntegration higgsfield) : ControllerBase
{
    ...
    public async Task<IActionResult> Login(CancellationToken ct)
    {
        ...
        // таком состоянии — обман. Текст должен называть рабочее действие: тумблера
        // «включить» в UI нет (McpServerList рисует Toggle только для своих), человек
        // может только выйти из Higgsfield и войти заново — Logout чистит запись, а
        // LoginAsync создаст её с Enabled=true.
        if (higgsfield.TryGetRecord(UserId) is { Enabled: false })
            return BadRequest(new { error = "Higgsfield выключен на нашей стороне. Кнопки «включить» для него в интерфейсе нет — выйдите из Higgsfield и войдите заново." });
