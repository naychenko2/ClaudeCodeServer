...
public static class FeatureFlagKeys
{
    ...
    // Автоправило архивации чатов (план «Архив чатов» v4, шаг 6): фоновый сервис раз в час
    // убирает в архив чаты без активности дольше выбранного порога. Флагом закрывается
    // ТОЛЬКО автоправило и его настройки; ручной архив, раздел «Архив» и сводка карточки
    // работают без флага.
    public const string ChatAutoArchive = "chat-auto-archive";
}
...
public static class FeatureFlagCatalog
{
    ...
    [
        ...
        // Автоправило архивации (план «Архив чатов» v4, шаг 6). Накопившиеся старые чаты
        // правило само не разгребает: первый проход запускает только кнопка «Применить
        // сейчас» в настройке правила, дальше правило работает фоном раз в час.
        new FeatureFlagDefinition(
            Key: FeatureFlagKeys.ChatAutoArchive,
            Title: "Автоправило архива чатов",
            Description: "Чаты без сообщений дольше выбранного срока сами убираются в архив — список остаётся чистым, а разговор сохраняется целиком. Закреплённые чаты и чаты с активными задачами остаются на месте. Ручной архив и раздел «Архив» работают и без этого тумблера.",
            Default: false,
            Stage: "dev"),
    ];
