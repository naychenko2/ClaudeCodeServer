...
- **Плавающее окно рендерится в `App`, над страницами** — единственный режим, который
  переживает переход между проектами. На время перетаскивания экран накрывается
  прозрачным слоем: чужие iframe съедают `pointermove` и рвут жест.
