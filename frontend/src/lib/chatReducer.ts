...
export function applyServerMessage<S extends ChatState>(prev: S, msg: ServerMessage): S {
  ...
  switch (msg.type) {
    ...
    case 'user_message': {
      // Сервер-инициированная отправка (автоматизация/задача) и ввод пользователя
      // с ДРУГОГО устройства: их клиент не добавлял реплику оптимистично, поэтому
      // они приходят живьём. Соединение-отправитель сервер исключает (GroupExcept),
      // так что дублей у инициатора нет и дедуп его не касается.
    ...
    case 'exited': {
      ...
      // workflowDone (успех по вотчеру workflow_progress — у workflow это независимый
      // канал завершения, он может прийти раньше структурного bg_agent_done).
      let bgClosed = false;
      const items = prev.items.map(item => {
        if (item.kind !== 'tool_use' || item.bgDone === true || item.workflowDone === true || !isBgLaunchResult(item.result)) return item;
        bgClosed = true;
