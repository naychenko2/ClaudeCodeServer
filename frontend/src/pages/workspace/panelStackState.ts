...
// Персист в localStorage — свои ключи cc_ws_panels_*, старые cc_artifacts_* не трогаем.
...
const KEY_LAYOUT = 'cc_ws_panels_layout';
const KEY_OPEN_LEGACY = 'cc_ws_panels_open'; // старый плоский список — мигрируется в layout
const KEY_WEIGHTS = 'cc_ws_panels_weights';
...
// ---------- модульный стор (паттерн — как lib/sidebarWidth.ts) ----------

let _layout: PanelKey[][] = parseLayout(lsGet(KEY_LAYOUT), lsGet(KEY_OPEN_LEGACY));
...
export function usePanelStack(): PanelStack {
