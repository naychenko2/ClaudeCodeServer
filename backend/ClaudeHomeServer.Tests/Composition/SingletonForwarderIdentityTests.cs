...
// Smoke-тест «форвардер сработал»: три пары, которые раньше регистрировались дублями
...
//
// Все три проверки обязательны: задача зафиксировала только TaskManager как «сломанный
// на проде», но PersonaManager и UserHomeResolver страдают от того же паттерна, и
// без явной проверки следующий регресс поймает только один из трёх.
