...
меньшинством). После доводки 2026-08-20 видно 80% значений — 68% как есть и 12% хэшем, —
а скрытые 20% скрыты осознанно. Добавляя логирование, сверяйся с этим списком, а не с догадкой.
...
Так и сделано с хвостом аудита: `{Key}` дев-сервера стал `{ServiceId}`, `{Key}` каталога
действий — `{Action}`, `{Rule}` — `{RuleId}` (в нём везде и лежал `rule.Id`, имя лишь намекало
на название от пользователя), `{Source}` маршрута — `{Route}`, `{Result}` доклада — `{Outcome}`.
Отдельный случай — `FallbackLlmSessionAdapter`: три его обёртки логирования складывали ВСЮ
диагностику подмен моделей в `{Message}`, поэтому наружу уезжало «[ModelFallback] {Message}» —
предупреждения о подменах, переполнении и исчерпании цепочки приходили пустыми, причём именно
они попадают в досье инцидента. Теперь это `{FallbackStep}`.

Закрытыми осознанно остались `{Name}`, `{Owner}`, `{Title}`, `{User}`, `{Raw}`, `{Value}`,
`{Old}`/`{New}`, `{Handle}`, `{Domain}`, `{Task}`, а также `{Key}` MCP-сервера (его slug
задаёт владелец), `{Source}` в `SkillsCliService` (туда идёт пользовательский ввод) и
`{RuleName}` (название правила сочиняет пользователь).
...
| Правило | Источник | Порог | Severity |
|---|---|---|---|
...
| Ходы стали медленнее | `duration.count` − бакет `le=1200000` | ≥ 2 ходов дольше 20 мин за 30 мин | info |
| Ходы массово встали | спаны `chat.turn` (трейсы), разрез по `chat_id` | ≥ 4 ходов дольше 15 мин в ОДНОМ чате за 2 ч | warning |
| Лимиты провайдера жмут | `ccs.llm.rate_limit_hits{status=rejected}` | > 5 за 30 мин | warning |
...
Скорость ходов наблюдают два правила разного назначения, и оба считают ХОДЫ, а не квантили.
**«Ходы стали медленнее»** (info) — индикатор «к сведению»: два и более хода дольше 20 минут
за полчаса, то есть похоже на зависание. Одиночный длинный ход для агентной работы норма,
поэтому одного мало.

Путь к этой форме стоит помнить, чтобы не пройти его заново. Сначала правило было p99 > 5 мин
за 15 мин и звенело почти на каждый длинный ход. Потом порог подняли до 10 мин, окно до 30.
Потом сменили `matchType` на `all_the_times`, рассчитывая отсечь одиночный всплеск, — правило
залипло в firing: SigNoz оценивает условие на своей, более грубой сетке, где p99 выше порога
во всех точках, хотя по минутным точкам он скачет от 0,5 до 20 минут. Корень в том, что ходов
в окне бывает ОДИН-ДВА, и на такой выборке p99 — просто самый долгий ход; никакой `matchType`
этого не лечит. Квантиль как основание для алерта здесь мёртв, и в правилах его больше нет.

**«Ходы массово встали»** (warning) ловит залипший ЧАТ, а не загруженную инсталляцию.
Считаются спаны `chat.turn` длиннее 15 минут (`durationNano >= 900000000000`), разрез —
`deployment.environment` + `chat_id`, порог «четыре и более» применяется к каждому чату отдельно.
Единственное правило на трейсах, а не на метриках: `chat_id` в метриках нет и не будет
(`ServerMetrics.AllowedTags`, кардинальность), а в спанах он есть и внесён в KEEP `PiiRules`.

Путь сюда — история двух ошибок. Сначала правило считало квантиль (p50 > 5 мин), и разбор
2026-08-19 показал, что при 1–2 ходах в окне медиана равна самому долгому ходу: «половина
ходов встала» означало «один ход был долгим». Квантиль заменили на ЧИСЛО ходов дольше
10 минут по всей инсталляции (`duration.count` минус бакет `le=600000`) — и эта форма сгорела
2026-08-21: четыре длинных хода в окне (24, 21, 17 и 10 минут) пришли из ШЕСТИ РАЗНЫХ чатов,
все успешные, ошибок LLM ноль, отклонённых по лимитам ноль. Правило сказало правду и не
значило ничего: сложенные в кучу чаты делают параллельную работу неотличимой от деградации.
Отсюда разрез по `chat_id` — шесть тяжёлых чатов сразу это нормальный вторник, а один и тот
же разговор, раз за разом молотящий по четверти часа, это симптом.

Тонкости формы. `matchType` обязан быть **`in_total`** — нужна СУММА за окно, а не отдельный
шаг (урок прежних редакций в силе). Окно и порог связаны арифметикой и двигаются только
вместе: четыре хода по 15 минут требуют минимум часа чистого времени, поэтому окно кратно
больше — два часа, двойной запас на паузы между ходами (прежние полчаса при любом пороге выше
единицы делали условие недостижимым в принципе). Фильтр `chat_id EXISTS` обязателен: спаны
без тега схлопнулись бы в одну безымянную серию, и разные чаты снова суммировались бы.

Порог прошёл две ступени калибровки на семи сутках трейсов (23 хода дольше 15 минут на бою).
Максимум в одном чате — три подряд, набранные 21.08 с 21:51; правило с порогом «три» зажглось
в тот же вечер и указало на чат верно, но работа там была штатной, просто тяжёлой. Порог
поднят до четырёх — на той же неделе он не сработал бы ни разу. Дальше двигать надо снова
ПОРОГ, а не границу: 15 минут уже вдвое больше типичного агентного хода с tool-use, и
опускать её значит ловить обычную работу.

**«Лимиты провайдера жмут»** (warning) закрывает дыру, найденную тем же разбором: 24
срабатывания rate-limit за полтора часа при НУЛЕ ошибок LLM. Ходы не падали — пул тихо
ротирует подписки, — поэтому «Всплеск ошибок LLM» молчал по существу, до `ccs.llm.errors`
дело не доходило. Видно было только следствие (ходы идут дольше), а причина не отражалась
нигде. Это ранний признак: когда ротировать станет некуда, ходы начнут падать и загорится
правило 02.

Первая редакция правила считала МЕТРИКУ ЦЕЛИКОМ — и потому звенела впустую: 20.08 она
зажглась на 29 событиях, среди которых не было ни одного отклонённого хода. Корень в самой
метрике: `TurnTelemetry.RecordRateLimit` вызывался на каждое `rate_limit_event`, а CLI шлёт
его ~на каждый ход и со `status="allowed"` — счётчик «срабатываний rate-limit» на деле мерил
АКТИВНОСТЬ, и порог «15 за полчаса» означал «15 ходов за полчаса». Те самые 24 события из
разбора 19.08 тоже были ходами, а не упорами в лимит, поэтому и калибровка «вдвое выше
наблюдавшегося темпа» опиралась на пустое. Теперь событие размечено тегом `status`
(`allowed` | `allowed_warning` | `rejected` | `unknown`), правило считает только `rejected`,
а сумма по всем статусам осталась приблизительным счётчиком ходов — но как сигнал не годится
и в алертах не используется. Порог 5 — первичная калибровка: отклонённый ход означает
исчерпанное окно подписки, единичные при переключении окон возможны; проверить порог по
накопленным данным стоит через неделю.

Разметка статусом стоила нового тега в `AllowedTags`, и это осознанно: множество значений
замкнуто протоколом CLI, поэтому лимитер `MetricTagGuard` тут не нужен — хватает белого
списка в `TurnTelemetry.ClassifyRateLimitStatus`. Смена набора тегов рвёт непрерывность
временного ряда: старые точки лежат без `status` и под фильтр правила не попадают, так что
после накатки правила сравнивать «до и после» по этой метрике нельзя.
