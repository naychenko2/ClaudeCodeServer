...
// «Применить» — единственная кнопка: дописывает изменённый порог и сразу
// прогоняет правило по сфере, иначе залежи так и лежат в списке (фоновый тик
// их не трогает).
...
export function ArchiveSettings({ initialDays, projectId = null, onArchiveDone }: Props) {
  // Локальный черновик порога: хранится в UI, в стор пишем тумблером и
  // «Применить». При первом включении правила
  ...
  // Идёт «Применить»: запись порога + проход правила по сфере. Отдельно от
  // saving (запись порога у тумблера) — у кнопки свой индикатор и свой текст
  // состояния.
  ...
  // Блок занят: идёт запись порога (тумблер) или проход правила («Применить»)
  ...
  // «Применить» = согласие на уборку: перед проходом дописываем изменённый
  // порог (правило идёт по тому, что человек видит в поле), затем запускаем
  // проход по этой сфере. Успех (в том числе «убрано 0») закрывает диалог,
  // ошибка — оставляет открытым и разблокирует кнопку, чтобы человек мог
  // повторить.
  const apply = async () => {
    setApplying(true);
    try {
  ...
  return (
    <div style={{
      ...
      {/* Поле порога и кнопки действий. Скрыто, когда правило выключено:
      ...
      {enabled && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {/* Единственная кнопка блока: «Применить» дописывает изменённый порог
              и сразу прогоняет правило по этой сфере. Пока идёт запись или
              проход — кнопка погашена; индикатор и текст состояния рядом,
              чтобы пауза не читалась как «ничего не произошло»: диалог закроется
              сам по завершении прохода. */}
          <Button
            variant="primary"
            size="sm"
            loading={applying}
      ...
      {/* Счётчик превью: сколько чатов этой сферы подпадёт под текущий порог.
          Проход по этому счётчику не запускается сам — его запускает «Применить». */}
      {enabled && (
