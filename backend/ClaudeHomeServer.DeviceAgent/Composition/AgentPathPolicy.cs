...
/// Проверка пути и открытие — два шага: подмену ссылки между ними (TOCTOU) ловит уже
/// сверка открытого дескриптора (<see cref="AgentOpenedPathGuard"/> в FileService).
/// Жёсткие ссылки по построению неотличимы от файла — от них защищает только то, что в
/// корень не пишут другие пользователи машины (проверка в <c>roots add</c>).
/// </summary>
