...
| Представление | Что отбирает | Когда открывать |
|---|---|---|
| Отказавшие ходы | `chat.turn` с `outcome = 'error'` | пришёл алерт «Всплеск ошибок LLM» |
| Медленные ходы | `chat.turn` дольше 30 с | пришёл алерт про деградацию скорости |
| Запуски в песочнице | `process.start` с `kind = 'docker'` | разбор «контейнер тормозит или нет» |

Каждое — прямое продолжение алерта: алерт говорит «что-то не так», представление
