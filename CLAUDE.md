...
- **Отказ ВЫХОДА В СЕТЬ отделён от отказа провайдера.** `Unreachable` покрывает оба, но лечатся
  ...
  (TCP-коннект к `HTTP(S)_PROXY`), спрашиваемая ТОЛЬКО при `Unreachable` и ДО кулдауна.
...
(десктопным чатам нельзя), при возврате целевой путь резолвится НА МОМЕНТ ВОЗВРАТА. Сводка
карточки — место `chat-digest` по кнопке, кэш инвалидируется активностью. Автоправило
«убирать чаты без активности N дней» — за флагом `chat-auto-archive`: пачка с
`ArchiveBatchId` и откатом ровно одного прохода; накопившиеся залежи разгребает только
кнопка «Применить сейчас». Не путать: архив ≠ временный чат (тот удаляется по сроку) ≠ чип
...
(2026-08); в каталоге **пять флагов**: `workspace-destructive` (постоянный предохранитель от
...
(настраиваемые секции промпта и типовые умения по специальности персоны) и
`chat-auto-archive` (автоправило архива чатов; ручной архив, раздел «Архив» и сводка
карточки работают без флага — см. раздел «Архив чатов»).
