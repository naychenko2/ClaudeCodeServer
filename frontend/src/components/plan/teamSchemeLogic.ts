...
// склоняют одинаково, делиться кодом с компонентом не будем. Общего модуля
// склонений в кодовой базе нет: та же копия живёт в useMcpData и lib/spend,
// локальная копия на модуль — действующая конвенция (проверено при ревью:
// заявленного lib/plural.ts не существует)
