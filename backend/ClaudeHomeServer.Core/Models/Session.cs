...
public class TeamImplementBudget
{
    ...
    // Обе точки считали ОДНО И ТО ЖЕ выражение Max + max(0, plan - left) (плашка — с
    // коммита 0ffaf081, лечения дефекта c156193b, где она до того брала за новый потолок
    // размер плана). Сведение в один метод — не починка расхождения, а защита от него:
    // копия формулы в двух файлах разъезжается тихо. Покрыто TeamImplementBudgetTests
    // (сама формула) и TeamWaveServiceTests.ПлашкаБюджета_* (плашка считает ею же).
