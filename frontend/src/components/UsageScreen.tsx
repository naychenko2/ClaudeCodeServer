...
function OllamaTab({ info }: { info: OllamaUsageInfo | undefined }) {
  ...
  const onLocal = info.actions.filter(a => a.routedToOllama).length;
    ...
    <div>
      {!info.enabled ? (
        <div style={{ padding: '10px 12px', marginBottom: 12, borderRadius: 8, fontSize: 12, lineHeight: 1.5,
          ...
          смогут считаться бесплатно на своей машине. Сейчас шаг локальной модели в цепочке пропускается.
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
          ...
          <MetricCard value={`${onLocal} / ${info.actions.length}`} label="действий на локальной модели" valueColor={C.textHeading} />
        </div>
      )}
