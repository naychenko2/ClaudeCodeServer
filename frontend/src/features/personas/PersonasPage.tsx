...
export function PersonasPage({ auth, onLogout, onHubTab }: {
  ...
  // Back/forward браузера внутри раздела «Персоны»: восстанавливаем выбор персоны.
  useEffect(() => {
    const onPop = (e: PopStateEvent) => {
      const s = e.state as NavSnapshot | null;
      if (s?.screen !== 'personas') return;
      setCreating(false);
  ...
  // «Специальности» — свой раздел хаба (#/specialties), вход из меню аватара.
  // Отсюда в него уводит мостик со специальностью в карточке персоны.
  const openSpecialties = () => onHubTab('specialties');
  ...
  const selectPersona = (id: string, view?: PersonaView) => {
    ...
    // Переход к проектной/не-глобальной персоне: в дефолтном «Глобальные» её нет в
    // списке, страж ниже молча сбросил бы выбор. Переключаем listMode на 'all',
    // чтобы персону стало видно. В диплинк-ветке выше это уже сделано в consume().
