...
| Ходы массово встали | `duration.count` − бакет `le=600000` | ≥ 3 ходов дольше 10 мин за 30 мин | warning |
...
**«Ходы массово встали»** (warning) ловит массовость — но НЕ квантилем. Прежняя редакция
(p50 > 5 мин) исходила из того, что медиана устойчивее p99; разбор 2026-08-19 показал, что
при 1–2 ходах в окне это неверно в принципе: медиана из одного значения равна этому значению,
и «половина ходов встала» означало «один ход был долгим». Теперь считается ЧИСЛО ходов дольше
10 минут — `ccs.llm.duration.count` минус бакет `le=600000` формулой `A - B`, порог «три и
более». Порог в штуках делает гейт
по размеру выборки ненужным: три медленных хода — это три хода, сколько бы их ни было всего.
Граница именно 10 минут: первая редакция считала ходы дольше 5 минут и зажглась сразу же на
обычной агентной работе (3 из 7 ходов за полчаса) — для хода с tool-use пять минут норма.

Три тонкости этой формы, все проверены на живых данных. Во-первых, `matchType` обязан быть
**`in_total`**: `increase` считает по минутным шагам и в каждой точке даёт максимум 1, так
что `at_least_once` с порогом 3 не сработал бы никогда — нужна сумма за окно. Во-вторых,
значения тега `le` — целые строки (`300000`, `600000`, `+Inf`), без научной нотации;
промах строкой обнулил бы `B`, и правило считало бы медленными ВСЕ ходы разом. В-третьих,
`A` берётся из `ccs.llm.duration.count`, а НЕ из бакета `le='+Inf'` той же метрики: при двух
фильтрах по одной метрике вычитание молча не выполняется — `F1` возвращает само `A`.
