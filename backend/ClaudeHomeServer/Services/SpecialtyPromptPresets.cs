...
public static class SpecialtyPromptPresets
{
    ...
    // Сквозная строка секции «процессы»: попутный баг вне задачи — не чинить молча и не
    // описывать в чате, а завести карточку-дефект и продолжить свою работу. Переиспользуется
    // в четырёх исполнительских ролях (см. ProcessesTexts); у Tester/Reviewer та же мысль
    // вклеена в конец «Куда складывать:» в узнаваемом формате «ситуация → tasks_create(...)».
    private const string DefectBugAside =
        "Попутный баг вне задачи → не чинить молча и не описывать в чате: tasks_create(kind:\"defect\", repro: шаги/ожидаемое/фактическое), продолжай свою работу.\n";
    ...
    private static readonly Dictionary<PersonaSpecialty, string> ProcessesTexts = new()
    {
        ...
        [PersonaSpecialty.Tester] =
            ...
            "Куда складывать: отчёт — в задачу (что проверено/что нет, как воспроизвести); найденный баг вне текущей задачи → tasks_create(kind:\"defect\", repro: шаги/ожидаемое/фактическое), а не абзац внутри отчёта.",
        [PersonaSpecialty.Planner] =
            "До сдачи плана:\n" +
        ...
        [PersonaSpecialty.Reviewer] =
            ...
            "Куда складывать: находки — ответом в чат или задачу; баг вне тронутого диффа → tasks_create(kind:\"defect\", repro: шаги/ожидаемое/фактическое), а не строка в находках.",
        [PersonaSpecialty.Secretary] =
            "До закрытия задачи:\n" +
