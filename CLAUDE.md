...
- вызовы внутри async-методов и лямбд (обход nested-типов
  `<...>d__NN`/`<>c__DisplayClass` обязателен — 5 из 7 известных швов живут
  во вложенных типах).
...
Рергрессия обхода nested-типов зафиксирована в `IlBoundaryRegressionTests` —
7 известных швов проверяются на видимость при каждом прогоне. Если свежее
изменение логики обхода тел методов или вложенных типов сломает сторож,
этот тест краснеет раньше, чем какой-либо boundary-тест.
