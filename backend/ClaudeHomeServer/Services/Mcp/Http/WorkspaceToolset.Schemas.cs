...
public sealed partial class WorkspaceToolset
{
    ...
    [
        ...
        Tool("projects_create",
            "Создать новый проект: на диске СОЗДАЁТСЯ папка в стандартном каталоге проектов "
            + "пользователя. Подключение существующей папки (rootPath) из MCP недоступно — "
            + "это делает человек через интерфейс.",
            Obj(new JsonObject
            {
                ["name"] = Str("Название проекта"),
                ["rootPath"] = Str("Не используется (зарезервирован): путь всегда выбирает сервер в стандартном каталоге"),
        ...
        Tool("chats_send",
            ...
            Obj(new JsonObject
            {
                ...
                ["preempt"] = new JsonObject
                {
                    ["type"] = "boolean",
                    ["description"] = "Прерывать ли идущий ход получателя (по умолчанию true — доклад "
                        + "уходит сразу по концу прерванного хода). false — НЕ прерывать рабочий ход: "
                        + "сообщение встанет в очередь и уйдёт после его штатного конца, поэтому ответ "
                        + "придёт позже — смотри chats_history и НЕ отправляй повторно (дедуп ловит дубли, "
                        + "но лучше не надо). Ход, ждущий подтверждения человека, прерывается ВСЕГДА — сам "
                        + "он не закончится. Замороженную «Стоп» очередь сообщение не размораживает: ждать "
                        + "разбора человеком",
                },
            }, "sessionId", "text")),
