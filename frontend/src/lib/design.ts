...
// CHAT_SCROLLBAR_W — ЗАПАС под полосу прокрутки ленты. Полоса идёт вплотную к
// правому краю сообщений (прокручивается колонка, а не вся ширина центра), а слева
// от колонки ставится ровно такой же паддинг — иначе колонка разошлась бы с
// композером, который центрируется сам по себе (геометрию считает chatBox.ts по
// ИЗМЕРЕННОЙ ширине полосы). Здесь величина нужна заранее и с запасом: у систем,
// тем и настройки «наложением» полоса разной ширины, реальный максимум ~17.
export const CHAT_SCROLLBAR_W = 20

// CHAT_COLUMN_W — сколько лента чата занимает ПО ГОРИЗОНТАЛИ целиком, вместе с
// обвязкой: колонка чтения + место под полосу справа + равный ему отступ слева.
//
...
// он посчитает последние 40px свободными и отдаст их под padding, и лента начнёт
// сжиматься раньше времени.
export const CHAT_COLUMN_W = CHAT_MAX_W + CHAT_SCROLLBAR_W * 2
