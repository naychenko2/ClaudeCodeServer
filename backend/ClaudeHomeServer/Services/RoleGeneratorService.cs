...
public class RoleDraft
{
    ...
    // Стартовые подсказки пустого чата («что поручить сотруднику»)
...
public class RoleGeneratorService
{
    ...
    private static string BuildSystemPrompt(IReadOnlyList<AgentInfo> agents)
    {
        ...
        sb.AppendLine("<ROLE>{\"name\":\"\",\"title\":\"\",\"avatar\":\"<один эмодзи>\",\"color\":\"<hex>\",\"persona\":\"\",\"agentNames\":[],\"systemPrompt\":\"\",\"model\":\"\",\"effort\":\"\",\"suggestions\":[]}</ROLE>");
        ...
        sb.AppendLine("Поле persona — 2-4 предложения: характер, манера речи, отношение к работе. Пиши живо и конкретно, из услышанного на собеседовании, а не шаблонно.");
        sb.AppendLine("Поле suggestions — РОВНО 3 коротких (4-8 слов) примера задач, которые пользователь может поручить именно этому сотруднику. Конкретные, из его специализации, сформулированы как поручения («Разбери…», «Придумай…», «Проверь…»).");
        if (agents.Count > 0)
        {
            sb.AppendLine("Поле agentNames — ОБЯЗАТЕЛЬНО подбери по специализации сотрудника подходящие fileName из доступных агентов (это его рабочие инструменты-компетенции). Не подходит ничего — оставь пустой массив, но сначала честно примерь каждый:");
            foreach (var a in agents)
                sb.AppendLine($"- {a.FileName}: {a.Description}");
            sb.AppendLine("Если подходящих агентов не нашлось (или их мало для специализации) — компенсируй полем systemPrompt: 3-5 предложений профиля навыков (что конкретно умеет, какими методами работает, что для него хороший результат).");
        }
        else
        {
            sb.AppendLine("Доступных агентов нет — agentNames оставь пустым массивом, а вместо них заполни systemPrompt: 3-5 предложений профиля навыков (что конкретно умеет, какими методами работает, что для него хороший результат).");
        }
        return sb.ToString();
    }
