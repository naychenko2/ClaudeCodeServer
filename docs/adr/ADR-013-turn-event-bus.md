...
| `turn/completed` | Notification | `FallbackLlmSessionAdapter` (`finally` цикла попыток) | `TurnRunLog` (этап 1: переезд записи паспорта хода с `finally` на подписчика); `SessionManager` (этап 4 / шаг 1в: боевой подписчик конца хода штаба — замещает прямой вызов `HandleTeamTurnEndAsync` из `OnMessageAsync`; план в `LastTeamTurnEnds` по ключу `TurnSeq`) | Паспорт хода. `Outcome` (success/failed/egress_down/**local_down**/interrupted/cancelled/crashed — семь исходов), `ErrorClass` (`TurnErrorClassifier`) у неуспешного исхода, `Passport` (полный `TurnRunPassport`) |
