...
export const api = {
  ...
  // Деньги Yandex Cloud: остаток на счёте (только админу) и расход на озвучку за N дней —
  // его продукт считает сам, у Яндекса такого разреза в API нет
  yandex: {
    account: (days = 30) => request<YandexAccountResponse>(`/yandex/account?days=${days}`),
  },
  ...
  video: {
    ...
    feed: (provider: string, channelId?: string, refresh = false) =>
      request<VideoFeedResponse>(
        ...
        { live: true }),
    // Избранные каналы полосы. live: true — как и у каналов: подставленный из офлайн-кэша
    // набор увёл бы полосу на чужой состав и первое же нажатие звёздочки сохранило бы его.
    favorites: () => request<VideoFavoritesResponse>('/video/favorites', { live: true }),
    setFavorites: (keys: string[]) =>
      request<VideoFavoritesResponse>('/video/favorites', {
        method: 'PUT', body: JSON.stringify({ keys }), live: true,
      }),
...
// Ссылки внешнего доступа — СКВОЗНЫЕ по проектам владельца, поэтому не под /projects/{id}:
  ...
  projects: {
    ...
    // на сервере (404 при выключенной фиче, Р11 плана). Грабли: дефолтный timeoutMs у
    // request() — 30 с (offline.ts:318), а ход модели идёт до минуты — без явного 180_000
    // обрыв покажется пользователю как «Действие недоступно офлайн» (образец — analyzePrompt
    // строкой выше). 409 на обоих путях — файл изменился между сканом и запросом, тело
    // несёт error: "staleBaseSha" и актуальный baseSha — фронт читает err.body.baseSha.
    mapHygiene: {
      scan: (id: string) =>
      ...
      apply: (id: string, baseSha: string, ids: string[]) =>
        request<MapHygieneApplyResult>(`/projects/${encodeURIComponent(id)}/map-hygiene/apply`, {
          method: 'POST', body: JSON.stringify({ baseSha, ids }), timeoutMs: 180_000,
        }),
    },
  ...
  personas: {
    ...
    // Подбор голоса по характеру. Без id: характер в момент подбора ещё не сохранён, а у
    // новой персоны id нет вовсе. voice=null в ответе — модель никого не выбрала (не ошибка)
    suggestVoice: (req: { name?: string; role?: string; description?: string; character?: string; tone?: string }) =>
      request<{ voice: string | null; role: string | null }>('/personas/ai/voice', {
        method: 'POST', body: JSON.stringify(req),
      }),
    // Личный голос персоны; пустой объект снимает его — персона снова говорит голосом инстанса
    setVoice: (id: string, voice: PersonaVoice | null) =>
      request<Persona>(`/personas/${encodeURIComponent(id)}/voice`, {
        method: 'PUT', body: JSON.stringify(voice ?? {}),
      }),
  ...
  sessions: {
    ...
    getContext: (projectId: string, sessionId: string) =>
      request<SessionContextEntry[]>(`/projects/${projectId}/sessions/${sessionId}/context`, { live: true }),
    // Идемпотентная замена состава целиком. Ответ PUT — записи БЕЗ missing (бэкенд его
    // считает только в GET), поэтому после записи дёргаем GET — он же обновит кеш
    putContext: async (projectId: string, sessionId: string, entries: Pick<SessionContextEntry, 'type' | 'id' | 'title'>[]) =>
      { await request<void>(`/projects/${projectId}/sessions/${sessionId}/context`, {
        method: 'PUT',
        body: JSON.stringify(entries),
      }); return api.sessions.getContext(projectId, sessionId); },
  ...
  // Сторожа чатов: чаты и проекты владельца с АКТИВНЫМИ сторожами. Снимок нужен потому,
  // что событие watchdogs_changed приходит только на смене состава — открывший список
  // позже постановки иначе о них не узнает
  watchdogs: {
    snapshot: () => request<{ sessions: string[]; projects: string[] }>('/watchdogs'),
  },
  ...
  tts: {
    // Каталог голосов для формы выбора: канонические имена, подписи, пол и амплуа.
