...
public static class FeatureFlagKeys
{
    ...
    // Визуальный разворот плана: контекстные замечания к разделам (часть A — слой маркеров
    // у заголовков, форма замечания, отправка через существующий onRespond). Часть B
    // (разворот схемой) будет под тем же флагом.
    public const string VisualPlan = "visual-plan";
    // Каталог MCP-серверов (план «Каталог MCP-серверов», волна 1): поиск по официальному
    // реестру registry.modelcontextprotocol.io и предзаполнение формы заведения из
    // декларации server.json. Гейтит поиск и приём CatalogRef при создании записи;
    // гейты безопасности у подключённых записей (подтверждение пробы/включения, SSRF)
    // работают всегда — безопасность не должна зависеть от тумблера фичи.
    public const string McpCatalog = "mcp-catalog";
    ...
    // Серверные сторожа чатов (план «chat-watchdogs»): модель декларирует «дожидайся
    // условия и разбуди этот чат» (watch_start), цикл бэкенда опрашивает переживая ходы
    // и смерть процесса CLI. Флаг гейтит доставку тулсета в ход и подсказку о нём.
    public const string ChatWatchdogs = "chat-watchdogs";
}
...
public static class FeatureFlagCatalog
{
    ...
    [
        ...
        // Визуальный разворот плана (план visual-plan, часть A): контекстные замечания к
        // разделам плана — маркеры у заголовков, форма замечания, отправка существующим
        ...
        new FeatureFlagDefinition(
            ...
            Default: false,
            Stage: "dev"),
        // Каталог MCP-серверов, волна 1 (поиск + предзаполнение формы). Текст — без
        // обещаний проверки чужого кода: каталог лишь источник предложения.
        new FeatureFlagDefinition(
            Key: FeatureFlagKeys.McpCatalog,
            Title: "Каталог MCP-серверов",
            Description: "Поиск сторонних серверов с инструментами (MCP) по открытому реестру сообщества: команда, адрес и имена ключей подставятся в форму сами — вам останется вписать свой ключ. AI Home не проверяет код этих серверов, смотрите, кто автор.",
            Default: false,
            Stage: "dev"),
        ...
        new FeatureFlagDefinition(
            ...
            Default: false,
            Stage: "dev"),
        // Серверные сторожа чатов (dark launch). Текст — без обещаний, что сторож
        // переживёт всё на свете: ограничение жизни и интервалы названы честно.
        new FeatureFlagDefinition(
            Key: FeatureFlagKeys.ChatWatchdogs,
            Title: "Сторожа чатов",
            Description: "Claude может оставить сторожа: сервер сам раз в интервал проверяет условие командой и будит чат, когда оно выполнилось (или когда истёк срок). Ожидание живёт на сервере и не зависит от того, закончился ли ход.",
            Default: false,
            Stage: "dev"),
    ];
