...
> Правило не пожелание, а гейт: размер карты и мёртвые ссылки в ней проверяет тест
> `ProjectMapHygieneGuardTests` (он зовёт сканер фичи «Уборка карты» на этом же файле).
>
...
**Наблюдение за деревом файлов не подписывается на служебные каталоги.** Оба потребителя
(ватчер хода и `FileWatcherService`) наблюдают через Core-примитив
[RecursiveDirectoryWatcher](backend/ClaudeHomeServer.Core/Services/Files/RecursiveDirectoryWatcher.cs):
ОДИН экземпляр inotify на наблюдателя и слежка по перечню каталогов, а не на всё дерево
(`FileSystemWatcher` с `IncludeSubdirectories` ставил её на КАЖДЫЙ каталог — 16 390 на этом
репозитории при 438 неслужебных). Сбои различаются намеренно: потеря событий лечится
пересинхронизацией, смерть наблюдения — пересозданием (отказ на отдельном каталоге —
частичный и пересоздания не поднимает: ровно оно дало рекурсию 19.09).

**Перед правками в примитиве или у его потребителей — прочитай
[docs/architecture/file-watching.md](docs/architecture/file-watching.md)** (обход шириной под
потолком слежек, списки исключений, символические ссылки, пересинхронизация на переполнении
очереди, идемпотентность гашения, маска событий).
...
## Локальные проекты (ADR-016)

Проект бывает серверным или **локальным** (`Project.DeviceId`): харнес и файлы живут на машине
пользователя под агентом устройства (`ClaudeHomeServer.DeviceAgent`), а всё внешнее
(LLM, MCP, выход в сеть) идёт через сервер. Инварианты:

- **Учётные данные сервера на клиент не едут никогда.** Шлюз `/gw/t/{turnId}/llm|mcp|egress`
  подставляет ключи сам. Токен хода знает только агент, он живёт столько же, сколько процесс
  CLI, и принимается только вместе с учёткой своего устройства. Подписки идут только
  `setup-token` и выключаются тумблером `LlmGateway:AllowSubscriptions`.
- **`ProjectCapabilities` — единственная точка правды** «что можно локальному проекту»:
  инлайновых `if (project.IsLocal)` не заводить. Серверная подсистема файловой группы
  отказывает ДО диска (сторож G10). Запуски — через `ILauncherFactory.ForProject`.
- **Ретранслятор чтения не умеет писать по построению**: в протоколе нет операций записи,
  запись — только агентом по localhost с машины проекта. Периметр localhost-API: один
  origin веб-морды (loopback-имена равны), билеты, сверка дескриптора (TOCTOU).
- Офлайн устройства: разовая автоматика ждёт («ждёт устройство»), периодическая пропускается;
  транскрипты локальных чатов живут только на устройстве. CLI на устройстве — своя копия,
  версию задаёт сервер.

**Перед правками — прочитай [ADR-016](docs/adr/ADR-016-local-projects.md) и
[device-agent-local-api.md](docs/architecture/device-agent-local-api.md)** (модель угроз,
билеты, ретранслятор, остаточный риск).
