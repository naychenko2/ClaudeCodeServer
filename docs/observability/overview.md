...
**Логи** (`PiiSanitizingLogProcessor`): тело сообщения пересобирается из ШАБЛОНА и уже
очищенных атрибутов. «Временный чат abc «Отчёт по клиенту» удалён» уезжает как «Временный
чат abc «{Name}» удалён»: разрешённый идентификатор виден, имя чата осталось
плейсхолдером, хэшируемый путь подставляется хэшем. Новых утечек рендер не создаёт по
построению — подставляется ровно то, что пропустил allowlist.

Так сделано не для красоты: экспорт идёт с `IncludeFormattedMessage` и `ParseStateValues`,
и до августа 2026 тело возвращалось к ГОЛОМУ шаблону. Значения лежали в атрибутах записи,
но ListView SigNoz показывает именно тело — и строка вида «cheap-runner: действие {Action}
— {Reason}» не сообщала ничего, пока не раскроешь каждую запись руками.
...
порог 10 мин и окно 30 мин отсеивают шум. Одиночного превышения тут мало по природе
выборки: при 7 ходах за полчаса p99 — это просто самый долгий из них, и правило зажигалось
от одного тяжёлого хода. Значения `at_least_n_times` не существует ни в одной версии
(`matchType` знает только `at_least_once`, `all_the_times`, `on_average`, `in_total`,
`last`), поэтому стоит **`all_the_times`** — превышение обязано держаться во всех точках
окна. Запасной ход, если это окажется залипчивым при скользящем окне `rate`: считать не
квантиль, а число ходов дольше 10 минут — формулой `ccs.llm.duration.count` минус бакет
`le=600000`; формулы SigNoz умеют только арифметику, так что гейт «p99 при count ≥ N»
в builder невыразим в принципе. **«Медиана ходов
