...
public class LlmProviderConfig
{
    ...
    // Белый список ИМЁН ИНСТРУМЕНТОВ внутри оставленного сервера: ключ — имя сервера (тот же,
    // что в KeepMcpServers и в сегменте маршрута /mcp/{name}), значение — инструменты, которые
    // сервер отдаёт этому провайдеру. Гранулярность нужна там, где сервер нужен целиком, а
    // отдельные его инструменты — нет: у tasks это tasks_delete (необратимое удаление) и
    // tasks_run_executor (порождение исполнителя), у memory — team_memory_* (запись в общее
    // знание команды, которое читают ВСЕ персоны). Плюс цена: замер 2026-09-06 показал, что
    // memory целиком стоит +2 984 токена промпта (+19.9%), а по делу из его 17 инструментов
    // нужны 8.
    //
    // Семантика: список для сервера задан → остаётся ТОЛЬКО перечисленное; не задан (или
    // пуст — пустой массив в конфиге неотличим от отсутствующего ключа) → сервер целиком,
    // как раньше. Сравнение имён — OrdinalIgnoreCase, как у KeepMcpServers. Неизвестное имя
    // в списке просто ничего не добавляет: состав — пересечение, а не объединение.
    //
    // Ключ — имя ТУЛСЕТА (сегмент /mcp/{name}), а не ключ сервера в конфиге хода: у памяти
    // это значит, что «memory» режет заодно все pmem_<handle> консультантов — они ходят тем
    // же маршрутом /mcp/memory/{personaId}/{projectId}. Ограничение: список работает только
    // на MCP-over-HTTP (продуктовые серверы); ветка отката на stdio (Mcp:HttpTransport=false)
    // и серверы личного реестра идут мимо контроллера и не фильтруются.
    //
    // Применяется В ОДНОЙ точке — McpTransportController (tools/list фильтрует состав,
    // tools/call отбивает отфильтрованный инструмент fail-closed); сами тулсеты о нём не знают.
    // Работает независимо от TrimMcpServers: это разные оси (какие серверы поднимать и что
    // отдаёт поднятый). Инвариант стабильности состава не нарушен — провайдер выводится из
    // EffectiveModel, свойства СЕССИИ, а не хода.
    ...
    // Нестабильные секции промпта (recall заметок и памяти, досье, привязки персоны,
    // граф кода) доставлять не системным блоком, а хвостом — вклейкой в текст хода.
    //
    // Зачем: у vLLM prefix cache совпадение ищется от начала контекста и обрывается на
    // первом изменившемся байте. Эти секции пересчитываются под текст каждого хода и
    // стоят в СЕРЕДИНЕ системного блока, поэтому обнуляют кэш всей истории следом за
    // собой. Замер по снимкам промптов прода 2026-09-16: у чатов локальной модели
    // совпадало 8 356 символов из 17 274 (медиана), обрыв на recall-notes в 22 случаях
    // из 38. Хвост контекста кэш не рвёт: текст хода и так новый каждый ход.
    //
    // Повтор не шлём: если склейка не изменилась против прошлого хода, она уже лежит в
    // транскрипте и доедет по --resume (см. ClaudeSession._lastTurnRecallHash).
    //
    // Дефолт false — облачные провайдеры не задеты: у них своя механика кэша
