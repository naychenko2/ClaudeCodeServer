...
`user|project|chat|task|persona|provider|model|source`), `from`, `to`, `scope`, фильтры.

Разрез `task` ортогонален `chat`: у задачи-исполнителя ровно один чат, поэтому цепочка
`task → chat` вырождена — пресет «По задачам» идёт `task → persona → turn`. Пустой ключ
разреза (`key: ""`) — «Вне задач» (обычные чаты и фоновые вызовы).
...
## GET /api/spend/tasks/{taskId}/prompt

Разбивка постановки задачи по секциям — из чего сложился промпт, отправленный исполнителю.
Показывается при раскрытии узла разреза «Задача». Владелец задачи или админ, иначе 403;
задача не найдена — 404.

**Только размеры в символах.** Ни текста постановки, ни описания задачи, ни заметок из базы
знаний здесь нет и быть не может — стор (`data/spend/task-prompts.jsonl`) хранит исключительно
числа (инвариант `TaskPromptMetricsStore`, под тестом-сторожем). Файл в облачный бэкап
не едет: это наблюдение, а не настройка.

Пустой `runs` — задача запускалась до появления учёта: не ошибка, блок разбивки просто
не показывается.

```jsonc
{ "runs": [                      // новые сверху; несколько записей = несколько запусков
  { "at": "2026-08-09T10:00:00Z", "taskId": "…", "ownerId": "…",
    "projectId": "…", "sessionId": "…", "personaId": "…",
    "totalChars": 1283,          // весь промпт постановки + блок заметок
    "totalTokensEst": 320,       // грубая оценка chars/4: занижает на кириллице,
                                 // для абсолютных токенов брать SpendStore
    "task": 55, "expected": 0, "tools": 0, "rules": 787, "restrictions": 0,
    "delegation": 252, "omo": 0, "context": 31, "notes": 0 }
] }
```
