...
// Общие примитивы разбора ответа LLM для autolearn (Этап 5, волна 2: вынесен в Core
// ради Dossiers — DossierCaptureService.ParseSummary использовал
// `Memory.MemoryLlmParsing.ExtractBalanced`, что было прямой ссылкой на тип из
// чужой вертикали). Извлечение сбалансированного JSON-фрагмента из «грязного»
// текста (преамбула/```-fence) и маппинг сырых записей в concrete-элементы
// стека. Персона и команда отличаются только enum'ом типа и типом элемента —
// задаются делегатами. Персона-специфичный разбор фокуса остаётся в
// персонном сервисе.
