// Раскладка режима «Фокус»: окрестность ОДНОГО типа вместо всего графа.
// Полный граф на 1020 узлах нечитаем (шаг между соседями кольца 1.3 px), поэтому
// как только узел выбран, холст показывает только его окружение: центр, слева —
// кто зависит от него, справа — от кого зависит он. Хвост сверх лимита уходит
// в заглушку «+N» и раскрывается списком в панели.
//
// Модуль чистый: считает координаты и ничего не знает про цвета и React —
// цвет ребра компонент берёт по relation из graphTokens (как в полном графе).
// Детерминизм тот же, что у graphLayout: никакого Math.random и force-симуляции.
