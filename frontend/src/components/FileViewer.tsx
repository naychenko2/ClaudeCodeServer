...
// Две ступени боковой колонки свойств и комментариев. Выше SIDE_COMPACT_W — полная
// (290px + зазор), документу остаётся ~450px. Ниже, до SIDE_MIN_W, — компактная
// (224px): в сплите с чатом панель файла попадает как раз сюда, и полные 290px
// оставляли бы документу лапшу в одно слово. Терпимо, потому что колонка плавающая
// и свёрнута в два заголовка: сжатие текста — только у верхушки документа, ниже
// он идёт во всю ширину (обтекание). Под SIDE_MIN_W колонка уезжает под текст
const SIDE_MIN_W = 570;
const SIDE_COMPACT_W = 760;
...
export function FileViewer({ project, filePath, onClose, onToggleFullscreen, fullscreen, isMobile, onOpenSidebar, initialTab, gitStagePath, scrollToLine, onOpenFile, scrollToAnchor, onFileBack, onFileForward, canFileBack, canFileForward, onTocChange, changedBy, onOpenChat }: Props) {
  ...
  // Компактная ступень колонки: полная ширина 290px, компактная 224px. В сплите панель
  // файла чаще всего попадает в диапазон SIDE_MIN_W..SIDE_COMPACT_W — колонка остаётся
  // сбоку от текста, но не отъедает у документа лишние 60px
  const sideCompact = !stackSide && panelWidth > 0 && panelWidth < SIDE_COMPACT_W;
  ...
  const docSide = (docProps.type || hasComments) ? (
    <aside style={stackSide ? {
      ...
      {/* Контейнер портала нужен только в режиме колонки: без комментариев панели нет */}
      {hasComments && <div ref={setSideEl} />}
    </aside>
  ) : null;
              ...
              editing
                ...
                : isHtml && htmlTab === 'preview'
                  ...
                  ? (
                    ...
                    // под текстом), свойства сверху. Панель комментариев уезжает в блок
