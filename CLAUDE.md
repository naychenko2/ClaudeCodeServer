...
`Boundaries`) три (Video, Yandex, Reader) уже вынесены в отдельные `.csproj` в
рамках Этапа 3 — это плановый мерж пилота, а не отмена исходного решения.
Остальные **17** без нового критерия не трогаем (профиль правок против — 89%
...
Статические конструкторы сторожей форсят загрузку всех вертикальных сборок
(`Video`/`Yandex`/`Reader`), иначе порядок тестов ложно рушит сторож при изолированном
прогоне (`IlBoundaryRegressionTests` — регрессия Этапа 3).
