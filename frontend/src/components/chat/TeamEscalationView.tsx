...
export function TeamEscalationView({ item, online }: {
  ...
  // уже рассчитал его через isWaiting && открытая по стадии карточка-блокер).
  // Здесь ещё раз стыкуемся по kind: на любой открытой карточке блокера при идущем
  // ходе строка появляется, у других видов (productDecision и пр.) — нет, иначе
  // при ходе по продуктовой развилке блокер-строка приписалась бы не блокеру
  const coordinatorHint = esc.kind === 'blocker' && ctx?.coordinatorTurnActive === true;
          ...
          {esc.actions.length > 0 && (
            <div style={{ display: 'flex', gap: SP.sm, flexWrap: 'wrap' }}>
              {esc.actions.map((a, i) => (
                ...
                >
                  {/* Подпись кнопки приходит с бэкенда в label (TeamEscalationActions.For) —
                      никаких хардкодов на id. Для drop бэкенд в этой же волне приводит
                      подпись к «Снять задачу у исполнителя», и старые транскрипты
                      (chosenActionId="drop") уже хранят её в label */}
                  {a.label}
                </Button>
              ))}
            </div>
