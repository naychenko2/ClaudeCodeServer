...
// пустое значение (например `subtasks: []`), чтобы модель не опускала поле
// вместо валидного «разбивать нечего». Держим рядом со схемами — правишь
// `required`, правится и подсказка.
...
Если в окружении доступен инструмент tasks_create — создай эти под-задачи в системе задач (по одной, с описанием), чтобы пользователь видел прогресс; в любом случае верни структуру. Отвечай по-русски.${requiredFieldsReminder(SUBTASKS_SCHEMA)}`,
...
Доведи до рабочего состояния: внеси правки, следуй стилю окружающего кода. Если в окружении есть tasks_update — отметь статус своей задачи (в работе → готово). Верни честный отчёт: если что-то не доделал — так и скажи. Отвечай по-русски.${requiredFieldsReminder(EXEC_SCHEMA)}`
...
Проверь текущее состояние git (ветки/worktree/diff), собери изменения в рабочее дерево, разреши конфликты по смыслу общей задачи. Верни отчёт о том, что сведено и какие конфликты были. Отвечай по-русски.${requiredFieldsReminder(EXEC_SCHEMA)}`,
...
Запусти релевантные проверки этого репозитория (например для фронта: cd frontend && npm run build; для бэка: cd backend && dotnet build / dotnet test). Если проверка красная — почини по месту (в пределах разумного) и перезапусти. Верни: прошло ли, что чинил, что осталось красным. Отвечай по-русски.${requiredFieldsReminder(VERIFY_SCHEMA)}`,
