...
// Ссылки внешнего доступа — СКВОЗНЫЕ по проектам владельца, поэтому не под /projects/{id}:
  ...
  // Сторожа чатов (флаг chat-watchdogs): чаты и проекты владельца с АКТИВНЫМИ сторожами.
  // Снимок нужен потому, что событие watchdogs_changed приходит только на смене состава —
  // открывший список позже постановки иначе о них не узнает
  watchdogs: {
