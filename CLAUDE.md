...
**Карточки дефектов** — тип карточки задачи (`Kind == "defect"`), не отдельный раздел:
сегмент «Задача | Дефект», шаги воспроизведения (`Repro`), вердикт проверки
(`Verification`) и исход (`Outcome`). Правила — `DefectRules` (чистые функции, зеркало в
`lib/tasks.ts`): дефект нельзя создать сразу в Done, закрыть без вердикта (кроме видимого
исхода «Закрыт без проверки» — деградация двух внутренних путей: галочка в заметке и
кнопка «Снять» волны штаба, иначе обе механики ломались бы запретом) и переместить в
колонку с ролью `review` без шагов (гейт по шагам действует только при наличии такой
колонки; гейт закрытия — всегда). Автор вердикта ставит сервер из `X-Caller-Session-Id`
(заголовок подделываем — это гигиена атрибуции, не защита). Продуктовые решения, тексты
интерфейса и «Что нового» — [docs/features/defect-cards.md](docs/features/defect-cards.md).
...
(2026-08); в каталоге **семь флагов**: `workspace-destructive` (постоянный предохранитель от
...
предзаполнение формы — см. раздел «Личный реестр MCP-серверов») и `chat-context`
(материалы — файл/ссылка/задача — закрепляются за чатом явной кнопкой: полоса вкладок у
чата плюс тул `context_list`; инварианты и критерии снятия — раздел «Контекст чата» в
[features.md](docs/architecture/features.md)).
