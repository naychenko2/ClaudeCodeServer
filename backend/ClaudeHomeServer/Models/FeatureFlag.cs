...
public static class FeatureFlagKeys
{
    ...
    // Режим чата «Командная реализация» (team-implement-mode): чат-штаб фичи — план с
    // исполнителями под каждую часть, задачи на персон, дочерние чаты исполнения, волны
    // и проверка.
    public const string TeamImplementMode = "team-implement-mode";

    // «Стена»: отдельный экран с несколькими чатами из разных проектов колонками рядом
    // (рельса набора, фокус, peek-панели сессии). Только десктоп ≥1200.
...
public static class FeatureFlagCatalog
{
    ...
    [
        ...
        // Режим чата «Командная реализация»: чат становится штабом фичи — план с исполнителями
        // под каждую часть, задачи, дочерние чаты исполнения под штабом, волны и проверка.
        // План согласовывается один раз, дальше команда работает сама в рамках бюджета.
        new FeatureFlagDefinition(
            Key: FeatureFlagKeys.TeamImplementMode,
            Title: "Командная реализация (режим чата)",
            Description: "Чат становится штабом фичи: план с исполнителями под каждую часть работы, задачи команде, чаты исполнения под штабом и проверка в конце. Согласуете план один раз — дальше команда работает сама и зовёт вас, только если застряла.",
            Default: false,
            Stage: "dev"),

        // «Стена»: 3-5 чатов из разных проектов рядом колонками — параллельное ведение
        // нескольких сессий. Слева рельса набора (монеты чатов), справа панели фокусного
        // чата по наведению. Работает только на широком экране (≥1200).
        new FeatureFlagDefinition(
            Key: FeatureFlagKeys.Wall,
            Title: "Стена",
            Description: "Несколько чатов из разных проектов рядом колонками: следите за параллельными сессиями, отвечайте на запросы разрешений и переключайтесь между разговорами, не покидая экрана. Доступна на широких экранах.",
            Default: false,
            Stage: "dev"),
    ];
