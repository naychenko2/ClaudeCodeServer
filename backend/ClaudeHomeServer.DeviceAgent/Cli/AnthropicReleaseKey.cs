namespace ClaudeHomeServer.DeviceAgent.Cli;

/// <summary>
/// Закреплённый в агенте публичный ключ подписи выпусков Claude Code
/// («Anthropic Claude Code Release Signing &lt;security@anthropic.com&gt;», RSA-4096).
///
/// Источник и отпечаток — официальная документация, раздел «Binary integrity and code signing»:
/// https://code.claude.com/docs/en/setup#binary-integrity-and-code-signing
/// (ключ раздаётся по https://downloads.claude.ai/keys/claude-code.asc, отпечаток
/// <c>31DD DE24 DDFA B679 F42D 7BD2 BAA9 29FF 1A7E CACE</c>).
///
/// Ключ НЕ качается в рантайме: скачанный по тому же каналу, что и манифест, он ничего бы
/// не доказывал. Смена ключа Anthropic = выпуск новой версии агента.
/// </summary>
public static class AnthropicReleaseKey
{
    public const string Fingerprint = "31DDDE24DDFAB679F42D7BD2BAA929FF1A7ECACE";

    public const string Armored = """
        -----BEGIN PGP PUBLIC KEY BLOCK-----

        
        9n3CmZUmbynZ+E/QWNENpW/pDNZdWFy43PASmML5FHu4m9Sn
        =oqMI
        -----END PGP PUBLIC KEY BLOCK-----
        """;
}
