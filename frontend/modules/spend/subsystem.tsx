// MF entry: реэкспорт манифеста подсистемы «Расходы» для Module Federation.
//
// Хост загружает этот модуль через loadRemote('aihome_spend/subsystem') и получает
...
// в catch и молча не регистрировал подсистему — раздел «Расходы» исчезал целиком.
//
...
export const subsystem: Promise<SubsystemManifest> = (async () => {
  ...
  const { manifest } = await import('../../src/features/spend/manifest');
  return manifest;
})();
