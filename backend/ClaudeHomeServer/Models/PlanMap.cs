...
public class PlanMapBlock
{
    ...
    /// <summary>0-based номер вхождения заголовка среди одноимённых: 0 — первое вхождение
    /// <see cref="Anchor"/> в плане, 1 — второе и т. д. Назначается сервером при сборке
    /// карты (модель считать вхождения не просят — делает ненадёжно): блоки идут по
    /// порядку массива и получают следующее свободное вхождение своего якоря. Нужен
    /// фронту для склейки замечаний и прыжка в раздел, когда в плане есть одноимённые
    /// заголовки (план из двух независимых задач).</summary>
