...
// Ссылки внешнего доступа — СКВОЗНЫЕ по проектам владельца, поэтому не под /projects/{id}:
  ...
  projects: {
    ...
    // Скан — мгновенно, без модели и без трат. Эта волна читает только факты; review/apply
    // за фич-флагом и закрыты на сервере (404 при выключенной фиче, Р11 плана).
    mapHygiene: {
      scan: (id: string) =>
