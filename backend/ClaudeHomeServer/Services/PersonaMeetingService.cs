    ...
    ILogger<PersonaMeetingService> log)
{
        ...
        string question, List<Persona> members, CancellationToken ct)
    {
        try
        {
            ...
            // --- Фаза 2: перекрёстная критика (каждому — позиции остальных) ---
            var positions = phase1.Where(e => !e.IsError).ToDictionary(e => e.PersonaId, e => e.Text);
            var jobs2 = alive.Select(p => (Persona: p,
                Question: "Перекрёстная критика на совещании. Вопрос совещания: " + question + "\n\n" +
                          "Позиции коллег:\n" + PositionsBlock(alive.Where(x => x.Id != p.Id), positions) + "\n\n" +
                          "Твоя прежняя позиция:\n" + positions[p.Id] + "\n\n" +
                          "Критикуй слабые места (1-3 пункта на оппонента), защищай своё, но ЯВНО уступай, " +
                          "если чужой аргумент сильнее твоего. В конце — твоя скорректированная позиция.",
                Context: (string?)null)).ToList();
            ...
            var synthesisQuestion =
                ...
                "Перекрёстная критика и скорректированные позиции:\n" +
                PositionsBlock(alive.Where(p => critique.ContainsKey(p.Id)), critique) + "\n\n" +
                "Сведи итог: 1) в чём консенсус; 2) оставшиеся разногласия с аргументами сторон; " +
                "3) твоя рекомендация — что делать. Пиши от своего лица.";
