...
пример формата, а ответ с маркером в примере обрывался бы на экране). Озвучка — `POST /api/tts`
...
браузера, менять осознанно. **Чей голос — одна точка, `VoiceResolver`** (голос персоны → голос
