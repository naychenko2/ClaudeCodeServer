...
public static class TeamImplementPrompts
{
    ...
    public static string CoordinatorTurn(SessionTeamImplement team)
    {
        ...
        // Правка плана на подтверждении: кнопкой «Изменить план» правку обрабатывает сервер
        // (ход координатору не приходит вовсе), а правка ТЕКСТОМ в чате обязана стать
        // перепланированием через маркер работы — без маркера новая версия плана не появится.
        else if (team.Stage == TeamImplementStage.Confirming)
            sb.AppendLine(PlanEditProtocol);
    ...
    public static string InterviewProtocol(SessionTeamImplement team)
    {
        ...
        sb.Append("— когда постановка ясна, заверши интервью маркером " +
                  ...
                  "по нему команда построит план и покажет его тебе и человеку. Конкретные значения " +
                  "из вводной и ответов человека (имена файлов, пути, точные строки) переноси " +
                  "дословно — не переформулируй и не заменяй своими.");
        return sb.ToString();
    }
    ...
    public const string WorkClassificationProtocol = """
        ...
        — нужно («добавь экспорт», «почини сортировку», «покрой тестами») — это работа: ответь
          ...
          по нему команда разложит работу на под-задачи и развернёт волну. Конкретные значения из
          вводной — имена файлов, пути, точные строки текста/содержимого — переноси в маркер
          дословно, без сокращений и подмен своими: сжимай формулировки вокруг них, а не сами
          значения (прод 2026-08-03: «hello5.txt» подменили на «smoke5.txt» — так нельзя);
    ...
    // Правка плана человеком на стадии подтверждения. Два канала: кнопка «Изменить план»
    // обрабатывается СЕРВЕРОМ (ход координатору не приходит вовсе — планировщик сразу
    // пересобирает план и публикует версию vN+1), а правка ТЕКСТОМ в чате — единственная,
    // что доходит координатору, и она ОБЯЗАНА стать перепланированием. Сформулировано так
    // же жёстко, как правило про вопросы через AskUserQuestion: ответ «план готов, жду
    // согласования» текстом — ровно та ошибка, что зависла на проде: новой карточки нет,
    // согласовывать нечего, а по старой можно запустить устаревшую работу.
    public const string PlanEditProtocol = """
        План ждёт подтверждения человека. Если человек написал в чат правку плана («поменяй…»,
        «тесты отдай Вере», «убери из плана…») — это запрос ПЕРЕСОБРАТЬ план:
        — ответь коротко, что принял правку, и в конце того же ответа поставь маркер
          <team:work>постановка для планировщика с учётом правки</team> — команда построит
          новую версию плана и покажет её на подтверждение. Конкретные значения из правки
          переноси дословно;
        — ЗАПРЕЩЕНО отвечать на правку только текстом («план готов, жду согласования», «учёл,
          подтверждайте»): без маркера новая версия плана не появится, и человек останется со
          старой карточкой, по которой можно запустить устаревшую работу.
        """;
    ...
    // волну) — зови человека, только когда своими силами не выходит. Канал зависит от природы
    // случая (решение по запрос владельца 2026-08-04 — единый язык вопросов в фиче):
    // — ВОПРОС в живом ходу (продуктовая развилка, любое уточнение по своей инициативе) —
    //   только AskUserQuestion, как в интервью: ASK блокирует ход до клика, ответ приходит
    //   в тот же ход и работа продолжается с того же места. Карточка с текстовым полем
    //   (маркер <escalate:decision>) больше не публикуется — самого маркера в протоколе нет,
    //   парсер терпит его только как фолбэк от молчаливого зависания;
    // — ОСТАНОВКА без вопроса (маркер в тексте, как маркер цикла «до готово»: состав
    //   MCP-инструментов не должен зависеть от режима хода) — когда нужно решение по кнопкам
    //   или работа встала вне хода координатора.
    public const string EscalationProtocol = """
        Если своими силами не справляешься — позови человека:
        — вопрос (нужно продуктовое решение между вариантами или хочешь что-то уточнить) задавай
          ТОЛЬКО инструментом AskUserQuestion с вариантами ответов (не пиши вопросы текстом) —
          человек получит карточку с кнопками, рекомендуемый вариант помечай, свободный ответ он
          даст через «Другое». Вопрос не останавливает работу: дождавшись ответа, продолжай
          с того же места;
        — маркером в конце ответа, когда нужно не спросить, а остановить работу:
    ...
    // которой не будет. У карточки есть кнопка «Повторить планирование» — повтор идёт по
    // сохранённой вводной, без хода координатору и без повторного интервью.
    public static string PlanFailedDetails(string request, string? reason)
    {
        ...
        sb.Append("Уточните задачу сообщением в чат или нажмите «Повторить планирование» — " +
                  "команда попробует ещё раз.");
        return sb.ToString();
    }

    // Отказ по таймауту планировщика (прод 2026-08-04): причина НЕ в постановке человека,
    // и текст обязан это сказать — иначе он переписывал бы нормальную вводную. Интервью
    // и вводная не потеряны: кнопка «Повторить планирование» строит план по той же вводной.
    public static string PlanTimeoutDetails(string request) =>
        $"Планировщик не уложился во время, разбирая вводную «{FirstLine(request)}». " +
        "Это не ошибка в постановке: вводная и ответы интервью на месте. " +
        "Нажмите «Повторить планирование» — команда построит план по той же вводной.";

    // Обрыв по лимиту вывода (прод 2026-08-05): модель выдала первые N токенов и заткнулась
    // на полуслове, парсер вернул null. Текст подсказывает, что делать: сократить вводную
    // или уменьшить число кандидатов. Таймаут тут не поможет — план и так отдался бы быстро.
    public static string PlanTruncatedDetails(string request) =>
        $"Планировщик не уместил план в лимит вывода, разбирая вводную «{FirstLine(request)}». " +
        "Скорее всего вводная слишком большая или в команде слишком много исполнителей. " +
        "Попробуйте сократить вводную или убрать часть исполнителей и нажмите «Повторить планирование».";

    // Невалидный JSON: модель отдала не JSON или JSON без под-задач. Не таймаут и не обрыв
    // — просто парсер не смог разобрать. Совет: повторить, модель может собраться с другой
    // формулировкой.
    public static string PlanInvalidJsonDetails(string request) =>
        $"Планировщик вернул неразборчивый план по вводной «{FirstLine(request)}». " +
        "Это сбой ответа модели, а не постановки. Нажмите «Повторить планирование» — " +
        "команда попробует построить план ещё раз.";

    // Сбой перепланирования по правке человека («Изменить план»): старая карточка уже
    // погашена как заменённая, поэтому молчать нельзя вдвойне — без этой карточки человек
    // остался бы вообще без плана. Правка не потеряна: повтор идёт по ней же.
    public static string PlanEditFailedDetails(string feedback, string? reason)
    {
        var sb = new StringBuilder();
        sb.AppendLine($"Правка «{FirstLine(feedback)}» не пересобралась в новую версию плана: " +
                      (string.IsNullOrWhiteSpace(reason) ? "планировщик не ответил." : reason.Trim() + "."));
        sb.Append("Старая версия плана погашена. Нажмите «Повторить планирование» — команда " +
                  "попробует пересобрать план по той же правке ещё раз.");
        return sb.ToString();
    }

    // Перепланирование + таймаут: отдельный текст, чтобы не винить правку в долгом ответе.
    public static string PlanEditTimeoutDetails(string feedback) =>
        $"Планировщик не уложился во время, пересобирая план по правке «{FirstLine(feedback)}». " +
        "Это не из-за правки: правка на месте. Нажмите «Повторить планирование» — " +
        "команда попробует пересобрать план по той же правке ещё раз.";

    // Перепланирование + обрыв по токенам: правка длинная, или в плане уже много под-задач.
    public static string PlanEditTruncatedDetails(string feedback) =>
        $"Планировщик не уместил новую версию плана в лимит вывода по правке «{FirstLine(feedback)}». " +
        "Скорее всего правка велика или в плане уже много под-задач. " +
        "Попробуйте сократить правку и нажмите «Повторить планирование».";
        ...
        string? actionLabel, string? comment)
    {
        ...
        // «Снять задачу» (drop) и «Пропустить» (skip): под-задача помечена Done тем же хуком,
        // SubtaskDropHandler, что и обычный доклад — координатор должен знать, что задача
        // закрыта, исполнитель получил отбой, и в работе её больше нет. Без этого явного
        // подтверждения координатор искал задачу в «активных» и предлагал перевыдать
        // (прод 2026-09, чат «Выделение подсистем для Француза»: человек снял задачу, а
        // координатор следующим ходом «продолжал работу»).
        if (actionId is "drop" or "skip")
        {
            sb.Append($"Задача по карточке снята ({actionLabel}), исполнитель получил отбой, " +
                      "в работе её больше нет. По этой ветке ничего не запускай и не перевыдавай; " +
                      "если без неё работа ветки стоит — закрой карточку-план или жди новой вводной.");
            return sb.ToString();
        }
    ...
    // Волна 2 team-blocker-honest: блокер почти всегда снимается действием координатора (а не
    // ответом человека), и текст должен явно перечислить ходы с их инструментами. Прежняя
    // формулировка «уточнить постановку, перевыдать работу, поправить план» была без имён
    // инструментов, и в чате «Выделение подсистем для Француза» координатор «снял блокер правкой
    // критерия», но Вере в её чат никто не написал — сторож зафиксировал молчащего исполнителя
    // и закрыл задачу как зависшую. Карточка гаснет сама по любому из этих действий (TryResolveBlockerByFactAsync
    // из волны 1), поэтому «позвать человека» — крайний случай, а не первая реакция.
    public static string BlockerReactionTurn(string? childChatName, string? childChatId) =>
        $"[Режим «Командная реализация»] Исполнитель{(string.IsNullOrWhiteSpace(childChatName) ? "" : $" «{childChatName}»")} " +
        "доложил о блокере — текст выше в ленте. Сними блокер действием в пределах бюджета: " +
        $"{(string.IsNullOrWhiteSpace(childChatId) ? "ответь исполнителю в его чат через chats_send (id чата возьми из контекста задачи);" : $"ответь исполнителю в чат через chats_send(sessionId=\"{childChatId}\");")} " +
        "уточни постановку через tasks_update (постановка, критерий готовности, файлы); " +
        "перезапусти исполнителя через tasks_run_executor; " +
        "перевыдай работу — заведи новую задачу через tasks_create и сними старую через tasks_complete " +
        "с пометкой о перевыдаче. Карточка блокера погаснет сама по любому из этих действий, " +
        "маркера эскалации ставить не нужно. Если блокер снят разговором, а не действием " +
        "(исполнитель уже отвязался, а вы просто подтвердили это в чате), поставь явный " +
        "маркер <team:resolved>кратко, что именно сделано</team> — карточка погаснет с подписью " +
        "«Снят штабом: …». Если снять блокер не выходит — поставь маркер эскалации " +
        "(<escalate:check> или <escalate:deviation>), тогда карточка останется человеку.";
