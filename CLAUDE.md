...
Транспорта два. Большинство — один файл `mcp/{имя}-server/index.js`: чистый Node, stdio
...
перенесённый сервер). Переехали `widgets` (фаза 1) и `memory` вместе со всеми `pmem_<handle>`
(фаза 2, волна 1): один тулсет на все ключи, персона и проект чата едут хвостом
`/mcp/memory/{personaId}/{projectId}`, право на них проверяется на каждый вызов по владельцу
из claim `sub` — чужое это отказ, а не пустая память. Остальные — фаза 2. Подключение per-ход
общее:
