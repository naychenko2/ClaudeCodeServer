...
Составил Александр (тимлид) по разведке выше. Ниже — решения по семи вопросам, вердикт по
порядку волны 4 и то, что выношу на решение Андрея.

**Что я перепроверил сам** (выводы стоят на этом, а не на пересказе разведки): сторож
`RootSubsystemBoundaryTests` целиком, таблица `Boundaries` в `SubsystemBoundaryTests`,
`SubsystemBoundaryCoverageTests`, `TaskManager.cs:21-37`, `Models/Session.cs:499-542`,
конструкторы всех шести файлов группы Chat, `BoardService.cs:9-19`,
`UnifiedSearchService.cs:11-12`, `TaskExecutionService.cs:94-105`,
`SessionManager.cs:6585-6590`, `MemorySubsystem.cs`, потребители всех пяти типов Specialty,
метрика `Program.cs` (1544 / 207), размеры неймспейсов `Services/*`. Факты разведки
подтвердились везде, где я их трогал. Ниже — три вещи, которых в разведке нет и которые
меняют арифметику решений.

### Три факта, найденных при проверке

**Факт А. Сторож корня не умеет точечных допусков — только префикс на всю вертикаль.**
У `SubsystemBoundaryTests` контракт двухуровневый: `AllowedNamespacePrefixes` (поддерево) и
`AllowedExactNamespaces` (ровно один тип по `FullName`) — именно поэтому Knowledge держит
11 точечных записей на корневые синглтоны, а не открывает себе весь корень.
У `RootSubsystemBoundaryTests` такого разделения **нет**: есть `SharedAllowedPrefixes`,
`RootAllowedSubVerticalPrefixes` (оба — префиксы) и `ExcludedRootTypes` (полное снятие типа
с проверки, «слепое пятно» по собственному комментарию сторожа). Следствие: как только
группа уезжает из корня в новую `Services.X`, у оставшихся в корне ссылок ровно два пути —
либо запись префикса `Services.X` в `RootAllowedSubVerticalPrefixes` (открывает корню
**всю** вертикаль целиком и по комментарию сторожа означает «это общая инфраструктура»,
чем Tasks и Notes не являются), либо `ExcludedRootTypes` (ещё хуже).

Это переворачивает главный тезис разведки о цене шага. Цена измеряется не числом рёбер:
со стороны **вертикалей-потребителей** переезд почти бесплатен — в `Boundaries` уже стоят
точечные записи на `TaskManager` (2 шт.), `PersonaManager` (5), `NotificationService` (2),
`NotificationStore`, `ChatHistoryService`, `PersonaMemoryService` (2), `TeamMemoryService` (2),
`NotesKnowledgeService`, `SessionMessagingService`, `FeatureFlagService`, `ProjectEventLogService`,
`UserHomeResolver`, `ProjectFileSessionsIndex`; после переезда это **правка строки**
(`Services.TaskManager` → `Services.Tasks.TaskManager`), а не новая запись. А со стороны
**корня** цена — одна грубая запись, размывающая сторож на целую вертикаль. То есть дорого
не то, что считала разведка.

**Вывод: перед любым выделением новой вертикали из корня сторожу корня нужен
`RootAllowedExactTypes` по образцу `AllowedExactNamespaces`.** Это ~30 строк в тесте, без
единой правки `.cs`. Без него волна 4C неизбежно превратит сторож корня в решето.

**Факт Б. Половина адресов переезда сама не под сторожем.** В `Boundaries` ровно 15 строк:
Video, Yandex, Reader, Images, Tts, Git, CodeGraph, Deploy, Backgrounds, ProjectIcons, Spend,
Dossiers, Knowledge, Watchdog, Memory. Реализаций `IAppSubsystem` — 13 (+ Watchdog вручную).
`SubsystemBoundaryCoverageTests` проверяет только направление «подсистема → строка в таблице»;
неймспейс `Services.X` **без** `IAppSubsystem` и без строки не ловится ничем (Watchdog внесён
руками именно поэтому). Вне сторожей сегодня:

| Неймспейс | Строк | | Неймспейс | Строк |
|---|---:|---|---|---:|
| `Services.Llm` | 18 496 | | `Services.Turn` | 1 114 |
| `Services.Prompts` | 5 539 | | `Services.Execution` | 1 035 |
| `Services.Docs` | 3 049 | | `Services.Modules` | 914 |
| `Services.Backup` | 2 080 | | `Services.TriggerSources` | 629 |
| `Services.Desktop` | 2 072 | | `Services.Diagnostics` | 161 |
| | | | **Итого** | **35 089** |

(`Services.Mcp` 11 336 не считаю — он в `SharedAllowedPrefixes`, спинка по конструкции.)

**35 089 строк без сторожа границ — это больше, чем весь корень Services (43 482) без
`SessionManager`.** И именно туда волна 4B предлагает переселить 7 450 строк. Формулировка
разведки «перенос внутрь `Llm` не требует ни одной записи в allow-list» верна буквально и
обманчива по сути: записей не требуется потому, что **проверки нет**. Переезд из корня (где
`RootSubsystemBoundaryTests` типы видит) в `Llm` — это не удешевление шва, это вывод кода
из-под контроля. Метрика «строк в корне» при этом улучшается — то есть метрика поощряет
ухудшение архитектуры. Это второй, независимый от арифметики довод против цели «<15k» как
критерия этапа.

**Факт В. Specialty нельзя двигать целиком — потребители у пяти файлов разные.** Разведка
рекомендует «увезти Specialty целиком в `Services.Llm`». Поимённая проверка потребителей вне
группы (без комментариев):

| Тип | Строк | Потребители вне группы | Адрес |
|---|---:|---|---|
| `SpecialtySettingsStore` | 1081 | `Llm`: `ModelAssignmentResolver`, `LocalActionRouter`, `PresetStore`, `GlmModelAliasMigration`; плюс `Turn`, `PersonaManager`, `PersonasCrudService`, `SessionManager`, `TaskExecutionService`, 4 контроллера | `Llm` |
| `SpecialtySettingsLayer` | 15 | `Llm/PresetStore`, 2 контроллера | `Llm` |
| `SpecialtyPromptPresets` | 476 | **только `SpecialtiesController`** | не `Llm` |
| `SpecialtyCatalog` | 141 | `PersonaBindingsService`, `PersonaConsultantToolset`, `TeamPlanningService`, 2 контроллера — **ноль в `Llm`** | не `Llm` |
| `SpecialtyTemplatesService` | 45 | `PersonasCrudService`, `Program.cs` | не `Llm` |

В `Llm` обоснованно едут **1 096 строк из 1 758**, остальные 662 — про секции промпта и
каталог специальностей персон (флаг `specialty-prompt-sections`), их потребители — Persona и
Team. Затащить их в модельный слой значит смешать «какая модель» с «какой текст» — ровно то
разделение, которое ADR-007 держит явно.

---

### Решения по семи вопросам

#### 1. `TaskManager` — вертикаль Tasks

**Решение: вертикаль, но выпуск шага обусловлен явным швом на три статических резолвера.**

**Обоснование.** Он владеет собственным стором `data/tasks.json` и событием `TaskCompleted`
— это домен, а не общий примитив; шесть из одиннадцати не-контроллерных потребителей уже
вертикали, и у двух из них точечная запись в `Boundaries` уже есть (переезд = правка строки).
Три «спинных» потребителя тонкие (57 / 198 / 97), причём один из них — `UnifiedSearchService`
— сам предмет вопроса №2. Аргумент «он уже в `ExcludedRootTypes`, значит признан backbone» я
отклоняю: комментарий самого сторожа (строки 160–162) прямо называет запись там слепым
пятном, заведённым по находке ревью `50f3b849`. Присутствие в списке исключений — это
свидетельство необработанного долга, а не архитектурного вердикта.

**Последствия и риски.** Проверил `TaskManager.cs:31-36` и `Models/Session.cs:499-542`:
три резолвера — публичные mutable-статики модели (`TaskSourceSessionResolver`,
`TaskDelegationDepthResolver`, `TaskDoneResolver`), от которых зависят вычисляемые
`Session.ParentSessionId`, `Session.TaskDelegationDepth` (гейт `TASKS_EXECUTE`!) и
`Session.TaskDone` (фильтр чатов). Формально после переезда связь остаётся законной
(`Models` в `SharedAllowedPrefixes`), но становится связью «вертикаль мутирует статику модели
спины в своём конструкторе», невидимой обоим сторожам. Риск не гипотетический: если
`TaskManager` однажды не будет создан к моменту первой сериализации `Session` — иерархия
чатов, гейт глубины делегирования и фильтр «Завершён» тихо выродятся в null/0/false, без
единой ошибки. **Требование к шагу Tasks:** либо узкий шов в спине (интерфейс вида
`ITaskFacts`, который назначает подсистема Tasks), либо, минимально, тест «после построения
хоста все три резолвера не null». Второе — три строки, и его достаточно для волны.

#### 2. `BoardService` и `UnifiedSearchService` — одно правило, два разных адреса

**Решение: правило одно — «фасад не остаётся в корне, если у его данных есть владелец;
владельца определяет источник ПЕРВИЧНЫХ данных, а не число зависимостей». По этому правилу
`BoardService` → вертикаль Tasks, `UnifiedSearchService` → остаётся в корне под точечным
допуском (Факт А), до появления третьего такого фасада.**

**Обоснование.** Разведка права, что решать надо одним критерием, но из этого не следует
одинаковый адрес — критерий на то и критерий, чтобы разводить непохожие случаи.
`BoardService` (проверил конструктор) асимметричен: первичные данные — задачи владельца,
сессии и персоны служат обогащением (статус колонки, подпись). Владелец есть → едет к нему;
его исходящие на `SessionManager`/`PersonaManager` — «вертикаль → спина», штатный допуск, обе
цели уже в списках. `UnifiedSearchService` симметричен по построению: заметки через
семантику Dify, задачи через ключевой поиск, оба — первичные. Владельца нет. Все четыре
варианта для него платные: в Tasks или в Notes — прямое ребро «вертикаль → вертикаль»
(запрещено ADR-014); в `Services.Mcp` вслед за `WorkspaceToolset` — нельзя, `Mcp` это
спинка со `SharedAllowedPrefixes`, класть туда продуктовую логику поиска значит прятать её
за границей, которой разрешено всё; отдельная вертикаль-агрегатор ради 97 строк — дороже
пользы. Остаётся корень под точечным допуском, и это дешевле всех перечисленных.

**Последствия и риски.** Тезис разведки «оставить в корне дороже, чем увезти» верен только
при сегодняшнем сторожа, где допуск бывает лишь префиксом на всю вертикаль. После введения
`RootAllowedExactTypes` (Факт А) он перестаёт быть верным: две точечные записи
(`Services.Tasks.TaskManager`, `Services.Notes.NotesService`+`NotesKnowledgeService`) —
именно та цена, которую и должен платить сшивающий тип. Явный триггер пересмотра: у
`UnifiedSearchService` в шапке заявлено расширение на чаты и файлы проектов; как только
доменов станет четыре либо появится второй такой фасад — заводить `Services.Workspace`
отдельной вертикалью-агрегатором со своей строкой в `Boundaries`. До тех пор новую
вертикаль ради одного файла не создаём.

#### 3. `TaskExecutionService` — этап 4, из волны 4 исключить

**Решение: в волну 4 не берём ни в каком виде. Место — этап 4, слой «приём хода / жизненный
цикл CLI-процесса».**

**Обоснование.** Проверил конструктор (`TaskExecutionService.cs:94-105`) — 17 зависимостей
подтверждаю, на группу Task приходится одна. Решающий довод не в цифрах, а в правиле: внутри
вертикали Tasks этот файл даёт ей прямые рёбра на Persona, Push, Notes и Specialty — это
**четыре нарушения** правила «вертикаль не зависит от другой вертикали напрямую», и
закрываются они не записями в allow-list, а отказом от правила. Ни один allow-list не делает
такую зависимость правильной; он делает её задокументированной. По природе это оркестратор
запуска Claude-исполнителя, живущий на `SessionManager.OnSessionMessage` — то, что цикл с
ядром уже обходится хуком `TeamWaveStarter` (`SessionManager.cs:6585-6590`, проверил), лишь
подтверждает: он давно часть session-слоя, а не задач.

**Последствия и риски.** В корне остаются 1 329 горячих строк (53 коммита за 3 месяца) —
метрика этапа 1 ухудшается ровно на эту величину, вертикаль Tasks в волне сжимается с 2 781
до 1 452. Шов между ним и `TaskManager` уже есть и он событийный: `TaskCompleted` →
`TryDeliverCompletionAsync` (комментарий `TaskManager.cs:17-18`), поэтому разрез проходит
чисто. Риск после выноса Tasks: корень будет ссылаться на `Services.Tasks.TaskManager` из
`TaskExecutionService` — снова упирается в `RootAllowedExactTypes`.

#### 4. Specialty + Subscription + Workflow — переезд в `Services.Llm` в предложенном виде отклоняю

**Решение: раздельно, и только после того, как `Llm` окажется под сторожем.**
- **Workflow (974)** → `Services.Llm`, принимаю без оговорок по составу.
- **Subscription (1 303) + `UsageService` (131)** → `Services.Llm`, но **после** заведения
  `LlmSubsystem` и строки в `Boundaries`.
- **Specialty — только `SpecialtySettingsStore` + `SpecialtySettingsLayer` (1 096)** →
  `Services.Llm`. `SpecialtyPromptPresets` (476), `SpecialtyCatalog` (141),
  `SpecialtyTemplatesService` (45) остаются в корне и уезжают с Persona (этап 4).
- **Новых вертикалей `Specialty`/`Subscription`/`Workflow` не заводим** — тут с разведкой
  согласен: три вертикали, чьи потребители целиком внутри одной чужой, это не модульность.

**Обоснование.** Поставленный в вопросе компромисс («ни одной записи в allow-list против
раздувания `Llm` до 22,5k») сформулирован на неполных данных. Записей не требуется не потому,
что шов чистый, а потому, что `Llm` не под сторожем вовсе (Факт Б): ни строки в `Boundaries`,
ни `IAppSubsystem`, ни покрытия coverage-тестом. Перенос 4 035 строк туда сегодня — это
перевод их из-под `RootSubsystemBoundaryTests` в зону, где границы не проверяет никто.
Раздувание при этом реально: 18 496 → 22 531 (+22%), и вертикаль становится крупнее, чем
весь остаток корня без `SessionManager` по моему плану (26 892 − 10 111 = 16 781). Оба этих минуса снимаются одним и тем же
предшагом — `LlmSubsystem` + строка в `Boundaries` с честным allow-list. После него переезд
действительно дёшев и правилен: механика фолбэка, ротации подписок и разрешения слотов —
это один домен, и разносить его по трём вертикалям было бы хуже.
Разбивку Specialty обосновывает Факт В: 662 строки из 1 758 не имеют в `Llm` ни одного
потребителя.

**Последствия и риски.** Предшаг «`Llm` под сторож» — не формальность и не бесплатен: у
`Llm` 18 496 строк и ссылки во все стороны, её первый allow-list будет длинным и, вероятно,
вскроет пару нежданных рёбер. Именно поэтому его надо делать **до** переезда, пока список
считается по 18 496 строкам, а не по 22,5k. Если предшаг не делается — Subscription и
Specialty не двигаем вообще: пусть лучше 2 530 строк остаются в корне под сторожем, чем
уедут в серую зону ради метрики.

#### 5. Группа Chat — подтверждаю, что группы нет; но её «расформирование» на этапе 1 неисполнимо

**Решение: вывод разведки подтверждаю и усиливаю. Однако строку прогноза «−497 через
расформирование Chat» снимаю — у пяти шестых этой группы нет адреса до этапа 4.**

**Обоснование.** Проверил конструкторы всех шести файлов, картина резче, чем в разведке:
`ChatArchiveService` (`SessionManager, ProjectManager, UserStore, FeatureFlagService,
NotificationService`), `ChatExpiryService` (`SessionManager`), `ChatTurnLoggerService`
(`SessionManager, ProjectEventLogService, ProjectManager, PersonaManager`),
`ChatTaskExtractionService` (`SessionManager, ProjectManager, Llm.ICheapTextRunner`) —
**четыре из шести держат `SessionManager` в конструкторе**; `ChatHistoryService` вообще без
зависимостей (файловый стор на одном `IConfiguration`); `ChatHistoryPaginator` — статический
класс с чистыми функциями над `Protocol`. Это не «группа с общим префиксом» — это
**три фоновые службы жизненного цикла сессии плюс стор плюс хелпер контроллера**. Общего у
них нет даже в природе, не только в имени.

**Последствия и риски.** Отсюда фактическая поправка к разделу «Прогноз метрики»: адрес
для `ChatArchiveService`/`ChatExpiryService`/`ChatTurnLoggerService` — session-слой, которого
до этапа 4 не существует. Переселять их в корне некуда, а заводить вертикаль «Chat» из трёх
служб `SessionManager` — значит создать вертикаль с тремя рёбрами в ядро. На этапе 1
реалистично уезжает один файл — `ChatTaskExtractionService` (149) вместе с прочим в `Llm`
(после предшага), и то по остаточному признаку. Значит последняя строка таблицы достижимости
(«14 606, запас 394») **не берётся**: −497 в ней необеспечены.

#### 6. `ExcludedRootTypes` — сокращать пошагово, как выходной критерий каждого шага волны

**Решение: отдельного «критерия сокращения списка» волне не нужно. Нужно жёсткое правило:
тип, покидающий корень, удаляется из `ExcludedRootTypes` тем же шагом, что и переезжает.
Типы, остающиеся в корне, не трогаем до этапа 4.**

**Обоснование.** Запись в `ExcludedRootTypes` снимает тип с проверки **и как проверяемый, и
как цель ссылки** (defense-in-depth, строки 244–247). Мёртвая запись после переезда опаснее
живой: она продолжает глушить проверку по `FullName`, и если в корне однажды появится тип с
тем же полным именем, он молча выпадет из-под сторожа — то самое слепое пятно, ради закрытия
которого сторож заводился. Держать «на всякий случай» запрещено прямым комментарием в самом
файле (строки 156–162, по итогам ревью `50f3b849`). По составу на волну приходятся:
`TaskManager` (шаг Tasks), `NotificationService` + `NotificationStore` (шаг Push, если он
вообще будет в этом этапе). `ChatHistoryService` остаётся — см. вопрос 5. `PersonaManager` —
только вместе с Persona. `SessionManager`/`ProjectManager`/`UserStore`/`FileService` — этап 4
и позже.

**Последствия и риски.** Снятие типа из `ExcludedRootTypes` делает видимыми **все** ссылки
корня на него — сегодня их не видит никто. Для `NotificationService`, который дёргают шесть
групп волны и четыре вертикали, это разом вскроет длинный список. Без `RootAllowedExactTypes`
(Факт А) закрыть его будет нечем, кроме префикса на всю вертикаль Push. Ещё одно
подтверждение, что правка сторожа — предшаг, а не сопутствующая работа.

#### 7. Ожидание по метрике — цель «корень < 15k» на этапе 1 не берётся, рекомендую заменить критерий

**Решение (рекомендация, финальное слово за Андреем — см. последний раздел): зафиксировать
до старта волны, что цель «<15k» на этапе 1 недостижима, и заменить критерий этапа на два
других, измеримых и не поощряющих ухудшение.**

**Обоснование — арифметика.** Разведка честно показала промах 2 397 по своему плану. Две мои
поправки его увеличивают:

| Шаг | Осталось в корне |
|---|---:|
| План разведки (4A+4B+4C) | 17 397 |
| + `TaskExecutionService` остаётся (вопрос 3) | 18 726 |
| − переселение хвоста спины 1 964 | 16 762 |
| − Briefing доезжает с Task (330) | 16 432 |
| − расформирование Chat (497) | **неисполнимо на этапе 1** (вопрос 5) |
| **Лучший честный результат этапа 1** | **16 432 — промах 1 432** |

Иными словами: даже полный этап 1 в максимальном объёме, включая все крупные группы, порог
не берёт. А по моему урезанному составу (следующий раздел) в корне останется ~26 900, и я
считаю это правильным результатом, а не провалом.

**Обоснование — почему сама метрика негодна.** Два независимых довода.
Первый уже назван разведкой: 69% остатка — один файл (`SessionManager` 10 111, 354 коммита
за 3 месяца), поэтому «строк в корне» меряет прогресс по `SessionManager`, а не по волне 4.
Второй — мой, по Факту Б: метрика **вознаграждает перенос кода в неймспейсы без сторожа**.
Волна 4B по своей сути — 7 450 строк из-под `RootSubsystemBoundaryTests` в зону, где границ
не проверяет никто. Критерий, который нельзя выполнить честно, но легко выполнить
формально, — плохой критерий.

**Что предлагаю вместо (для этапа 1):**
1. **Ноль неймспейсов `Services.*` вне сторожей границ.** Сегодня их десять на 35 089 строк
   (таблица в Факте Б) — дыра больше корня без `SessionManager`. Критерий проверяется тестом,
   а не глазами: расширить `SubsystemBoundaryCoverageTests` со «каждая `IAppSubsystem` имеет
   строку» на «каждый неймспейс `ClaudeHomeServer.Services.X` имеет строку в `Boundaries`
   либо запись в явном списке-исключении».
2. **Среднее число файлов на фичу ≤ 3** — заявленная в ADR-014 цель модульности,
   замеряется по git тем же способом, что и метрика `Program.cs`.
3. Цель «корень < 15k» — **перенести в критерий этапа 4**, где она достижима честно:
   расщепление `SessionManager` снимает 10 111 строк, и порог берётся с запасом.

---

### Вердикт по предложенному порядку волны 4

Порядок «4A чистые → 4B переселение → 4C со склейками» как последовательность **принимаю**:
он отсортирован по правильному признаку и обкатывает конвейер на безопасном. **Состав режу**
и **добавляю обязательный шаг 0 перед всем остальным.**

#### Шаг 0 (новый, блокирующий) — привести сторожей в состояние, при котором волна не портит границы

Только тесты, ноль правок `.cs`:
1. `RootAllowedExactTypes` в `RootSubsystemBoundaryTests` — точечные допуски корня по
   `FullName`, по образцу `AllowedExactNamespaces` (Факт А). Без него каждое выделение
   4C оплачивается открытием целой вертикали корню.
2. `LlmSubsystem` + строка в `Boundaries` для `Services.Llm`; следом — `Docs`, `Turn`,
   `Prompts`, `Execution` как адреса переездов 4B и переселения хвоста спины (Факт Б).
3. Расширить `SubsystemBoundaryCoverageTests` до «неймспейс без строки — красный тест».

**Почему блокирующий.** Без пункта 1 шаги 4C и вынос Push технически неисполнимы иначе как
через размывание сторожа. Без пункта 2 вся волна 4B — перекладывание 7 450 строк в серую
зону; метрика улучшится, контроль границ ухудшится. Это единственное место, где я
категорически возражаю против плана как он есть.

#### 4A — принимаю целиком, с одной поправкой

ProjSvc + Terminal (2 912), Changelog (630), DocAi (237) = **3 779**. Ноль входящих из спины
подтверждаю по разведке, вопросов нет. Поправка: у каждой новой вертикали 4A **строка в
`Boundaries` — часть определения готовности шага**, иначе она рождается вне сторожей (как
`Llm` сегодня). Для DocAi это существенно: «присоединить к существующей `Services.Docs`» —
адрес в серой зоне, пока `Docs` не получит строку (пункт 2 шага 0).

#### 4B — принимаю в урезанном составе

| Что | Куда | Строк | Решение |
|---|---|---:|---|
| ImgTail | `Services.Images` | 781 | **Да**, без условий: `Images` под сторожем, префикс уже в `RootAllowedSubVerticalPrefixes`. |
| Memory-хвост Persona/Team | `Services.Memory` | 2 634 | **Да**, без условий: `Memory` под сторожем, `MemorySubsystem` их уже регистрирует, шапка сама зовёт положение временным. |
| Workflow | `Services.Llm` | 974 | **Да, после шага 0.2.** |
| Subscription + `UsageService` | `Services.Llm` | 1 434 | **Да, после шага 0.2.** Без него — не двигать. |
| Specialty — только `SettingsStore` + `SettingsLayer` | `Services.Llm` | 1 096 | **Да, после шага 0.2.** Остальные 662 строки остаются (Факт В). |
| | | **6 919** | |

#### 4C — режу вдвое

| # | Группа | Строк | Решение |
|---|---|---:|---|
| 9 | Task **без** `TaskExecutionService`, **c** `BoardService` | 1 452 | **Да**, после шага 0.1. Плюс шов на три резолвера `Session` (вопрос 1). |
| — | Briefing доезжает с Task | 330 | **Да**: его единственный не-контроллерный потребитель `TaskSchedulerService` — внутри Task. |
| 10 | Notes | 2 777 | **Да**, после Task: шов `NoteTaskSyncService` снимается порядком. |
| 11 | Skills | 1 333 | **Да**, порядок свободный: один шов `SessionManager` → `SkillsService`. |
| 12 | Team (без memory-хвоста) | 1 936 | **Нет в этом этапе.** |
| 13 | Persona (без memory-хвоста) | 5 311 | **Нет в этом этапе.** |
| 14 | Push | 718 | **Нет в этом этапе.** |
| | | **5 892** | |

**Почему режу Persona, Team и Push — это главное расхождение с планом.**
*Persona* держится тремя вещами, и все три — не про волну 4. Восемь типов, которые дёргает
`SessionManager`, — это разговор про расщепление ядра, то есть этап 4; семь DTO из
`Controllers` в `PersonasCrudService` — самый длинный такой список в проекте (у Knowledge их
три), и он означает, что граница «сервис/контроллер» в Persona не проведена вовсе; 19 типов
из `Memory` частично снимаются шагом 4B, но только частично. Выносить 5 311 строк и 173
коммита за три месяца, зная, что через этап 4 половину швов придётся переделывать, — это
двойная работа с высоким шансом конфликтов. Persona надо делать **после** расщепления
`SessionManager`, а не до.
*Team* технически готова (хук `TeamWaveStarter` — настоящий шов, проверил), но она зависит
от Task четырьмя типами и от Persona — и её главная ценность, memory-хвост, уезжает бесплатно
шагом 4B. Оставшиеся 1 936 строк при 54 коммитах не окупают отдельной вертикали прямо сейчас.
*Push* разведка сама ставит «строго последней, после того как выехали шесть групп» — а по
моему составу пять из шести не выезжают. Значит её очередь просто не наступает: вынос
потребует снять два типа из `ExcludedRootTypes` и составить список потребителей, который
придётся переписывать на этапе 4. Работа ради 718 строк холодного горизонтального сервиса.

#### Что вообще не стоит делать (прямо, без вежливости)

- **Гнаться за метрикой «<15k» на этапе 1.** Три из четырёх оставшихся способов её добрать
  (Chat, переселение хвоста спины в `Turn`/`Prompts`, слив Specialty целиком в `Llm`) ведут в
  неймспейсы без сторожей или не имеют адреса. Цель закрывается формально и ухудшает предмет.
- **Заводить вертикали `Specialty`, `Subscription`, `Workflow`** — согласен с разведкой,
  но по другой причине: не «потому что дороже allow-list», а потому что вертикаль, все
  потребители которой внутри одной чужой вертикали, — это не модуль, а перенесённая папка.
- **Класть `TaskExecutionService` в Tasks** ради целостности группы по имени. Четыре
  запрещённых ребра «вертикаль → вертикаль» не лечатся allow-list.
- **Выносить Persona в этом этапе.** Повторю отдельно: это самая горячая группа проекта, и
  делать её до расщепления `SessionManager` — значит гарантированно переделывать.

#### Итог по объёму рекомендуемого этапа 1

Шаг 0 (тесты) + 4A 3 779 + 4B 6 919 + 4C 5 892 = **16 590 строк вынесено**, в корне
остаётся **26 892** (с переселением хвоста спины — ~24 900). Порог «<15k» не берётся и не
ставится целью. Взамен закрываются: серая зона сторожей (35 089 строк под контроль), пять
новых вертикалей с честными границами и точечный механизм допуска, без которого этап 4
резал бы `SessionManager` в неконтролируемое пространство.

---

### Что выношу на решение Андрея

**Развилка (вопрос 7 — про ожидания владельца, не про технику).** Три варианта, моя
рекомендация — второй:

| | Вариант | Что получаем | Чего не получаем |
|---|---|---|---|
| A | **Этап 1 в полном объёме по карте** (все 16 групп, включая Persona/Team/Push) | ~16 400 в корне; формально ближе всего к «<15k» | Порог всё равно не взят (промах 1 432); Persona и Team придётся переделывать после этапа 4; 7 450 строк уезжают в неймспейсы без сторожей |
| B | **Урезанный этап 1: шаг 0 + 4A + 4B + Task/Notes/Skills** (рекомендую) | Сторожа приведены в рабочее состояние, 35 089 строк серой зоны под контроль, 5 новых честных вертикалей, 16 590 строк из корня | Метрика «<15k» не закрыта и снята с этапа; в корне остаётся ~26 900 |
| C | **Прыжок сразу в этап 4** (расщепление `SessionManager`) | Настоящий выигрыш по «фича ≤3 файла» раньше | Резать 10 111 строк ядра без `RootAllowedExactTypes` и без `Llm` под сторожем — резать в неконтролируемое пространство; шаг 0 всё равно придётся сделать первым |

**Рекомендую B.** Причина коротко: этап 1 в его нынешней постановке пытается закрыть метрику,
которая меряет не ту величину, и платит за это выводом кода из-под сторожей. Урезанный этап 1
делает то, что реально нужно перед этапом 4: чинит инструмент контроля границ и выносит то,
что выносится честно и однократно. Настоящий выигрыш по заявленной цели модульности лежит в
этапе 4, и B — кратчайший корректный путь к нему.

**Три решения, которые нужны от Андрея явно:**
1. **Снимаем ли цель «корень Services < 15k» с этапа 1** и заменяем на «ноль неймспейсов
   `Services.*` вне сторожей» + «файлов на фичу ≤3»? Если цель остаётся как есть — этап 1
   закончится формально невыполненным при любом составе работ (арифметика в вопросе 7).
2. **Делаем ли шаг 0 (правки трёх тестов) блокирующим предшагом волны?** Это единственный
   пункт, где я возражаю против плана категорически: без него волна 4B ухудшает контроль
   границ, а 4C неисполнима иначе как через размывание сторожа корня.
3. **Согласен ли перенос Persona / Team / Push / `TaskExecutionService` за пределы этапа 1**
   (в этап 4)? Это минус 9 294 строки от плана разведки — самая заметная моя правка состава,
   и она напрямую определяет, сколько этап 1 займёт по времени.

**Фактов, которых мне не хватило для решения, нет** — все семь вопросов закрыты на
проверенных данных. Единственное место, где решение обусловлено, а не окончательно: адрес
`UnifiedSearchService` (вопрос 2) зависит от того, состоится ли шаг 0.1; если сторож корня
точечных допусков не получит, ответ меняется на «увозить в Tasks и терпеть ребро на Notes»
как меньшее зло.

**Правок в разведку не вношу.** Её факты подтвердились везде, где я проверял. Две поправки
по существу — необеспеченная строка «−497 через расформирование Chat» в разделе «Прогноз
метрики» (вопрос 5) и рекомендация «увезти Specialty целиком в `Llm`» (Факт В) — изложены
здесь и в исходном тексте не тронуты.
