...
> персоны со всеми надстройками, OMO-механики, workspace-tools, офлайн и т.д.) признаны готовыми
> и включены **безусловно** — гейты сняты; пометки «за флагом …» в описаниях этих фич выше —
> исторические. Актуальный состав каталога смотри в `FeatureFlagCatalog.All`
...
> к документам, локальные подсказки AI, панели проекта, переключатель проектов, виджеты в чате
> `chat-widgets`). Новый флаг заводится по инструкции ниже.
