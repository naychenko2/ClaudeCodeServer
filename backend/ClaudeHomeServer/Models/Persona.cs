...
public class Persona
{
    ...
    // Короткий slug (латиница/цифры/дефис) — @упоминания, роутинг сабагентов, Dify-датасет памяти.
    // Уникален В КОНТЕКСТЕ: проектные персоны — в рамках своего проекта, глобальные — глобально
    // (см. PersonaManager.MakeUniqueHandle).
    public string Handle { get; set; } = "";
    // Handle задан пользователем вручную — миграция и авто-переименования его не трогают
    public bool HandleCustom { get; set; }
    ...
    // УСТАРЕЛО (15.08.2026, упрощение модели): уровень модели персоны выведен из цепочки
    // приоритета (ModelAssignmentResolver его не читает) и убран из формы персоны. Поле
    // оставлено в модели данных, чтобы старые бэкапы personas.json и внешние клиенты не
    // падали: при загрузке стора заданное значение мигрируется разворотом его уровня
    // по действующей цепочке в TierMedium, поле очищается (PersonaManager.MigratePersonaModelTiers).
