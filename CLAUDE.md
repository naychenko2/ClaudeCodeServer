...
`Boundaries`) пять (Video, Yandex, Reader, CodeGraph, Skills) уже вынесены в
отдельные `.csproj` в рамках Этапа 3: сначала плановый мерж пилота (первые три),
затем по критерию готовности швов — CodeGraph (две волны) и Skills (две волны).
Остальные **15** без нового критерия не трогаем (профиль правок против — 89%
...
вертикальных сборок (`Video`/`Yandex`/`Reader`/`CodeGraph`/`Skills`), иначе порядок
тестов ложно рушит сторож при изолированном прогоне. В `IlBoundaryRegressionTests` статического
