...
import type { Project, GitFileChange, GitLogEntry } from '../types';
import { api } from '../lib/api';
import { C, R, FONT, MODAL_W } from '../lib/design';
import {
  useGitState, ensureGit, loadUnpushedLog, loadGitRemote,
  gitStage, gitUnstage, gitDiscard, gitCommit, gitPush, gitFetch, gitPull, clearGitError,
} from '../lib/git';
import { splitPath, relTime } from './GitPanel';
import { getExtMeta } from './FileExplorer';
import { Modal, ModalActions, TextArea, IconButton, Button, Menu, MenuItem } from './ui';
...
export function GitChangesRail({ project, onOpenDiff, onOpenFile, onOpenCommit, activeFilePath, onCommit }: Props) {
  ...
  const [chatMenu, setChatMenu] = useState<'scope' | null>(null); // дропдаун способов фиксации
  ...
  // Файлы выбранного коммита (read-only) — грузим при активации скоупа-коммита
  useEffect(() => {
    if (activeScope === 'working') { setCommitFiles([]); return; }
    let alive = true;
    void api.git.commitDetail(project.id, activeScope)
      .then(d => { if (alive) setCommitFiles(d.files); })
      .catch(() => { if (alive) setCommitFiles([]); });
    return () => { alive = false; };
  }, [activeScope, project.id]);

  const selectScope = (scope: 'working' | string) => { setActiveScope(scope); setMode('list'); };
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      {/* Шапка: ветка + fetch/pull/push + шестерёнка (промпт) */}
      <div style={{ padding: '10px 10px 8px', display: 'flex', alignItems: 'center', gap: 4 }}>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, flex: 1,
          background: C.bgWhite, border: `1px solid ${C.border}`, borderRadius: R.lg, padding: '5px 9px', minHeight: 30,
        }}>
          <GitBranch size={ICON_SIZE.xs} strokeWidth={ICON_STROKE} color={C.accent} style={{ flexShrink: 0 }} />
          <span title={status?.branch ?? undefined} style={{
            fontFamily: FONT.mono, fontSize: 12, fontWeight: 600, color: C.textHeading,
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1,
          }}>{status?.branch ?? '—'}</span>
          {(ahead > 0 || (status?.behind ?? 0) > 0) && (
            <span style={{ fontFamily: FONT.mono, fontSize: 10.5, fontWeight: 600, color: C.textSecondary, flexShrink: 0, display: 'flex', gap: 3 }}>
      ...
      {mode === 'commit' ? (
        ...
        <div style={{ borderTop: `1px solid ${C.border}`, background: C.bgInset, padding: '6px 8px', flexShrink: 0 }}>
          ...
          >
            ...
            <span style={{ flex: 1, fontSize: 12, color: isWorking ? C.accent : C.textHeading }}>Не зафиксировано</span>
            {/* Split-кнопка «Зафиксировать ▾»: основная часть → форма, дропдаун → выбор способа */}
            {isWorking && workingFiles.length > 0 && (
              <div style={{ position: 'relative', flexShrink: 0, display: 'flex' }}>
                <button
                  onClick={e => { e.stopPropagation(); void openCommitForm(); }}
                  style={{ fontSize: 11, fontWeight: 600, color: C.onAccent, background: C.accent, border: 'none', borderRadius: '6px 0 0 6px', padding: '3px 8px 3px 10px', cursor: 'pointer', whiteSpace: 'nowrap' }}
                >Зафиксировать</button>
                <button
                  onClick={e => { e.stopPropagation(); setChatMenu(chatMenu === 'scope' ? null : 'scope'); }}
                  title="Способы фиксации"
                  style={{ display: 'flex', alignItems: 'center', background: C.accent, border: 'none', borderLeft: `1px solid ${C.accentSoft}`, borderRadius: '0 6px 6px 0', padding: '0 4px', cursor: 'pointer' }}
                >
                  <ChevronDown size={12} strokeWidth={ICON_STROKE} color={C.onAccent} />
                </button>
                {chatMenu === 'scope' && (
                  <Menu onClose={() => setChatMenu(null)} align="right" top={-118} minWidth={180}>
                    <MenuItem icon={<Pencil size={15} strokeWidth={ICON_STROKE} />} label="Самому"
                      onClick={() => { setChatMenu(null); void openCommitForm(); }} />
                    <MenuItem icon={<MessageSquare size={15} strokeWidth={ICON_STROKE} />} label="В текущем чате"
                      onClick={() => { setChatMenu(null); onCommit?.('chat'); }} />
                    <MenuItem icon={<MessageSquarePlus size={15} strokeWidth={ICON_STROKE} />} label="В новом чате"
                      onClick={() => { setChatMenu(null); onCommit?.('newChat'); }} />
                  </Menu>
                )}
              </div>
            )}
          ...
          {/* Стек незапушенных коммитов: до 3 видно, дальше — внутренний скролл */}
          {st.unpushed.length > 0 && (
            <div style={{ maxHeight: 3 * 34, overflowY: 'auto', marginTop: 2 }}>
