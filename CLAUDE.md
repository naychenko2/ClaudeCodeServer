...
- **Признак «канал играбелен» вычисляется из ответа** (`streams != null`), а не списком id:
  свой поток есть лишь у каналов ВГТРК, остальные 17 из каталога вещаются плеером «Витрины»
  по домену-реферреру и уводятся ссылкой наружу. Реферер не подделываем.
- Каталог из 20 id зашит таблицей (эндпоинта-списка у сервиса нет, `/channels` → 404);
  названия в нём — фолбэк, **отказ одного канала не роняет список**.
...
- **Отдельного раздела хаба у видео НЕТ** — панель рельсы `video` (эфир фоном; полоса
  ...
  разворачивают крупно или выбирают канал в каталоге. Центр видео всегда во ВСЮ
  ширину (и кадр, и каталог): split рядом с лентой не прижился — разворачивают
  кадр, когда хотят смотреть, а фоновый просмотр закрывает панель. Выбор канала
  ...
  ушёл, — два живых iframe одного эфира = два звука. Кнопки паузы нет нигде:
  у прямого эфира паузы не бывает, снятый кадр вернётся в текущую минуту.
- **Живой кадр рисуется НАД страницами, а не внутри них.** Плавающее окно — `VideoFloat`,
  ...
  Пропал слот (ушли в раздел без панели, панель закрыли) — кадр гаснет не сразу, а через
  отсрочку в 5 с: перемонтаж страницы укладывается в кадр-другой, а приглушить эфир на
  это время нельзя — плеер СМОТРИМ команд не слушает. На время перетаскивания окна экран
  накрывается прозрачным слоем: чужие iframe съедают `pointermove` и рвут жест.
