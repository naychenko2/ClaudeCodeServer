...
Замерено на тихом стенде 2026-09-22 (`DFLASH_MAX_LEN=262144`, один прогон на уровень):

| занято, ток | prefill, ток/с | decode, ток/с |
|---|---|---|
| 60 095 | 1794 | 123.4 |
| 139 223 | 1295 | 121.3 |

**Вывод: decode по занятости окна практически не деградирует** — 123.4 против 121.3 ток/с при
удвоении контекста (−1.7 %). Падает только prefill (−28 %), а он на обычном шаге агента идёт
из prefix cache. Значит у раннего автосжатия остаётся ровно одна выгода — длина самой паузы,
и она оплачивается ростом амортизированной цены сжатий; цифры 85 → 50–70 → 34 ток/с из
`appsettings.json` описывают ПАРАЛЛЕЛЬНЫХ агентов, а не занятость окна. Уровень 230k в этот
прогон не попал: запрос на нём буксовал дольше выделенного окна на стенде.
...
Ручка проверена живьём 2026-09-22 (CLI 2.1.276): при `CLAUDE_CODE_AUTO_COMPACT_WINDOW=121192`
(расчётный порог 100 000) автосжатие сработало на `preTokens = 98 402` — отклонение −1.6 %,
сторож формулы зелёный. Контрольный прогон на той же нагрузке без ключа дошёл до 110 604
занятых токенов **без единого сжатия**: штатный порог там 232 248. Ключ доезжает до процесса
CLI через `ExtraEnv` — снято с `/proc/<pid>/environ` живого процесса.
