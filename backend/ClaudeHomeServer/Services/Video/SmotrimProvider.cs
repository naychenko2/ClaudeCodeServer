    ...
    ILogger<SmotrimProvider> log) : IVideoProvider
{
    ...
    // Одновременных запросов к чужому API: каталог из двух десятков каналов, лупить их
    // разом невежливо и легко получить троттлинг.
    ...
    private async Task<IReadOnlyList<VideoChannel>> BuildAsync(CancellationToken ct)
    {
        ...
        // Иначе три канала, которые реально можно смотреть, теряются среди семнадцати ссылок.
