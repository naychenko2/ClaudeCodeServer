...
public class Project
{
    ...
    // deny-ветка, умирает со снятием флага mcp-allowlist: пока флаг выключен, действует
    // именно deny-list — новый сервер владельца доезжает во все его проекты сам, а
    // пустой/null список = «включено всё, что включено в реестре». При зачистке флага
    // поле удаляется, а инкремент BackupSchema.Version 5→6 страхует откат.
    // Ось каскада реестр → проект → персона (см. SessionManager.BuildExternalMcpProvider).
    public List<string>? McpServersOff { get; set; }
    // Ключи серверов личного реестра MCP (McpRegistry), включённых В ЭТОМ проекте —
    // allow-ветка, действует за флагом mcp-allowlist. null/пустой список = «не включён
    // никто»: при allow-модели сервер доезжает в проект только при явном включении.
    // deny-ветка McpServersOff умирает при снятии флага, а это поле остаётся.
    // Оба поля сосуществуют весь период флага; действующее определяется флагом владельца.
